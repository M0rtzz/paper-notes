...
**本文要解决什么？**（1）设计一种 LiDAR 场景中的智能物体放置机制，使 copy-paste 增强在点云中可行；（2）处理部分标注场景中的假阴性问题（未标注物体被当作背景训练）；（3）构建一个不依赖伪标签的 LiDAR SSOD 框架。
...
**核心idea一句话**：用 Transformer 网络预测未标注 LiDAR 场景中物体的合理放置位置，首次使 copy-paste 增强在 LiDAR SSOD 中可行，配合空间感知损失处理假阴性问题。
