...
**本文要解决什么？** (1) 如何智能地选择信息量最大的 token 进行掩码预训练？(2) 如何确保可见 token 集合包含足够丰富的语义信息以支持鲁棒预测？(3) 如何在保持预训练效率的同时提升模型对不同场景的泛化能力？
...
**核心idea一句话**：通过强化学习驱动的动态采样策略选择信息性 token，结合从完整场景到掩码场景的自蒸馏知识传递，实现更鲁棒的轨迹预测表示学习。
