...
**本文要解决什么?** 如何在编辑医学图像时精确控制修改区域，同时保留不应改变的关键特征，从而生成可靠的合成数据集来量化模型对三类数据集偏移（采集偏移、表现偏移、人群偏移）的鲁棒性。

**切入角度**: 引入两类掩码（edit mask 和 keep mask），将 CFG 仅限于编辑区域内，编辑区域外使用无条件生成以保证全局一致性，keep mask 区域强制恢复原始像素。

**核心idea一句话**: 通过双重掩码机制解耦虚假关联，实现对医学图像的精准可控编辑，进而系统性地诊断生物医学视觉模型在各种数据集偏移下的失败模式。
