...
description: >-
  [ECCV 2024][医学图像][脑信号解码] 提出UMBRAE，通过通用脑编码器将fMRI信号与图像特征对齐后送入冻结的MLLM，实现多模态脑解码（描述、定位、检索、视觉重建），并创新性地引入跨被试训练策略，使单一模型服务多个被试且优于单被试模型。 领域现状：脑信号解码研究近年取得进展，可将fMRI信号解码为图像（Min…
tags:
  - "ECCV 2024"
  - "医学图像"
  - "脑信号解码"
