...
description: >-
  [ECCV2024][语义分割][图像分割] 提出 Cs2K 框架，从类别特有知识（原型引导伪标签 + 原型引导类别适应）和类别共享知识（权重引导选择性整合）两个方面协同缓解增量语义分割中的灾难性遗忘与新类欠拟合问题。 增量语义分割（ISS）需要模型在学习新类别的同时保持对旧类别的分割能力。现有方法存在两类偏差： 1. 缺乏…
tags:
  - "ECCV2024"
  - "语义分割"
  - "图像分割"
