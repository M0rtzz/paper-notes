...
**本文要解决什么？** (1) 验证 SNN 是否真的对 GIA 免疫；(2) 设计一种能有效攻击 SNN 的梯度反演方法；(3) 提出特征约束策略提升重建质量。

**切入角度**：作者发现 SNN 的随机训练过程可以被形式化为传统 NN 训练过程的一个变体——随机分量可以看作网络权重的一部分，从而将 SNN 的梯度反演问题转化为传统 NN 的梯度反演问题。

**核心idea一句话**：将 SNN 的随机训练过程重新参数化为传统 NN 的等价形式，使现有 GIA 技术可直接应用于 SNN，证明前向传播中的随机扰动无法保护隐私。
