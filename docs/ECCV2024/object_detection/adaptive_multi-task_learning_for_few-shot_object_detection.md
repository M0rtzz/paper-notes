...
**本文要解决什么？** (1) 如何有效调节分类和定位任务之间的梯度冲突，避免一个任务的优化损害另一个任务？(2) 在样本极度稀缺的情况下，如何借助外部知识（大规模预训练模型）来增强各个子任务的性能？

**切入角度**：作者提出从梯度层面动态调整两个任务的学习节奏——当一个任务表现较差时增大其梯度权重，反之减小，从而实现精度驱动的自适应平衡。同时利用 CLIP 的视觉-语言对齐能力来增强小样本场景下的分类精度。

**核心idea一句话**：用精度驱动的梯度平衡器从优化层面解决分类⁻定位冲突，用 CLIP 知识蒸馏从知识层面增强小样本分类能力。
