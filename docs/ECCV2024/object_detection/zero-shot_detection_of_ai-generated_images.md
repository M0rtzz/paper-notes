...
description: >-
  [ECCV 2024][AI生成图像检测] 提出基于熵的零样本检测器ZED，利用无损图像编码器建模真实图像分布，无需合成训练数据即可检测AI生成图像
tags:
  - ECCV 2024
  - AI生成图像检测
  - 零样本检测
...
**本文要解决什么？** 设计一种不需要任何AI生成训练数据、也不依赖对生成架构知识的零样本检测方法，能够自动泛化到任意新出现的生成器。
...
**核心idea一句话**：用无损图像编码器建模真实图像的像素概率分布，通过测量待检测图像的"意外程度"（交叉熵）来判断其是否为AI生成。
