...
**本文要解决什么？** (1) 构建一个涵盖多种噪声等级的真实世界事件去噪数据集；(2) 设计一个能够在不同噪声水平下都有效工作的事件去噪模型；(3) 验证方法在实际应用场景（如低光微观成像）中的有效性。
...
**核心idea一句话**：构建真实多噪声等级数据集 ED24，并设计 Transformer 架构 EDformer 来学习事件流中的时空相关性，实现对不同噪声水平的统一去噪。
