...
description: >-
  [ECCV2024][多模态VLM][数据清洗] 提出 InfoGrowth，一种高效的在线数据清洗与选择算法，通过近邻搜索估计每个样本的信息增益，实现数据集的持续增长，同时保证清洁度和多样性，在 CC3M 上仅用 1/6 数据即超过全量训练效果。 领域现状：深度学习依赖大规模数据，但网络数据以指数级增长（2024年每天产生…
tags:
  - "ECCV2024"
  - "多模态VLM"
  - "数据清洗"
