...
1. **中间层 Token Embedding 作为 Prompt Query**:
    ...
    - 做什么：用骨干 ViT 第 $l$ 层的 [CLS] token $q_l = x_{l_{[CLS]}}$ 直接作为 prompt query，替代原先需要额外 query ViT 前向传播的 $q = Q(x)_{[CLS]}$
...
2. **逐层 Prompt 生成（CodaPrompt 式加权求和）**:
    ...
    - 做什么：对第 $l$ 层，计算 query $q_l$ 与 prompt pool keys $\{k_l^1, ..., k_l^M\}$ 的 cosine similarity $\gamma(\cdot)$，加权求和得到 prompt $\phi_l = \sum_m \gamma(q_l, k_l^m) p_l^m$
...
3. **Query-Pool Regularization (QR) Loss**:
    ...
    - 做什么：在训练时通过 reference ViT 提取最终层 [CLS] token $r$，约束中间层 query 与 prompt pool 的相似度分布逼近 reference 的分布
