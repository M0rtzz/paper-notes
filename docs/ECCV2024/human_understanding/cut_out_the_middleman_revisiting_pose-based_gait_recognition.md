...
**本文要解决什么？**（1）找到一种更丰富的姿态编码方式，在保持姿态方法对衣物鲁棒的同时恢复形状信息；（2）设计跨数据集泛化的方案，消除对特定姿态估计器的依赖；（3）弥合基于姿态方法与基于轮廓方法之间的性能鸿沟。
...
**核心idea一句话**：用姿态估计器输出的热力图取代骨架坐标作为步态识别的输入表示，配合姿态引导对齐和全局-局部融合网络，使基于姿态的方法首次接近轮廓方法的性能。
