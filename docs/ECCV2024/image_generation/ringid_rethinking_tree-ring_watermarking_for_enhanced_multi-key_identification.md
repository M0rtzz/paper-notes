...
**本文要解决什么？** (a) 揭示 Tree-Ring 鲁棒性的真实来源；(b) 暴露其在多密钥识别中的严重缺陷；(c) 设计一种系统性方案提升多密钥识别能力。

**切入角度**：从数学推导证明分布偏移因子为 $\frac{\sqrt{3}}{2}$，并通过实验验证其在不同攻击下的贡献差异。

**核心idea一句话**：通过多通道异构水印框架融合不同类型水印的互补优势，配合离散化、无损嵌入和更圆环形等改进，系统性解决 Tree-Ring 在多密钥识别中的失败问题。
