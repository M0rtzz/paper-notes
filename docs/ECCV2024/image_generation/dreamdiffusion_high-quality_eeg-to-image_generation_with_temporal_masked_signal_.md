...
**本文要解决什么？** (1) 如何利用大量无标注EEG数据（无需配对图像）学习鲁棒的EEG表征？(2) 如何用极少的EEG-图像配对数据将EEG空间与CLIP文本-图像空间对齐？
...
**核心idea一句话**：用时序掩码预训练从大规模噪声EEG数据中学习鲁棒表征，再用CLIP图像编码器搭桥将EEG空间与SD的文本-图像空间对齐，实现从EEG直接生成高质量图像。
