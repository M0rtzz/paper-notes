...
**本文要解决什么？** (1) 如何在手部运动预测中平衡多样性和精度；(2) 如何利用未来信息引导扩散模型学习更精确的预测；(3) 如何通过prompt机制增强模型对观测运动的理解。

**切入角度**：在训练阶段利用ground truth未来运动作为引导信号，训练一个参考数据生成网络(RDGN)来模拟这种引导。推理时RDGN生成"替代未来数据"来引导预测。同时从观测运动中提取交互式prompt，提供额外的运动上下文。

**核心idea一句话**：通过"未来驱动"机制（训练时用GT引导，推理时用生成的参考数据替代）和基于观测的交互式prompt来引导扩散模型进行精确的手部运动预测。
