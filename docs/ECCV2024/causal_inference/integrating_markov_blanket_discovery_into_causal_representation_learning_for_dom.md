...
**本文要解决什么？**（1）如何在隐因果空间中定义和发现一个理论上最优的预测特征子集？（2）如何确保这个特征子集在不同域之间保持不变性？（3）如何在端到端框架中同时实现因果表示学习和最优特征选择？
...
**核心idea一句话**：在隐因果空间中发现目标变量的马尔可夫毯特征，用这些最小充分特征构建不变预测机制，实现比仅用因果/反因果变量更优的域泛化。
