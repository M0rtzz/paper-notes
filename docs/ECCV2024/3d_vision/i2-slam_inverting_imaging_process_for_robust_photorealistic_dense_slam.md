...
**本文要解决什么？** 在SLAM框架内直接对物理成像过程建模，使系统能从退化输入中恢复清晰的HDR地图和准确轨迹。具体分解为：（1）如何建模运动模糊过程以恢复清晰表示？（2）如何处理帧间曝光和白平衡变化？（3）如何将这些模块无缝集成到已有SLAM流程中？
...
**核心idea一句话**：通过反转物理成像过程（运动模糊积分+色调映射），将HDR线性辐射场作为SLAM的核心表示，联合优化场景、位姿和成像参数。
