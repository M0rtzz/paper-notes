...
description: >-
  [ECCV 2024][部件级控制] 提出 PartCraft，首次实现了基于部件选择的文本到图像生成控制——用户可以从不同物体中"挑选"各部件（如鸟的头、翅膀、身体），模型将它们自然地组合为一个全新且结构合理的创意物体。 当前生成式 AI（如 Stable Diffusion）的创意控制手段主要依赖文本描述或草图…
tags:
  - "ECCV 2024"
  - "部件级控制"
  - "文本到图像生成"
