...
**本文要解决什么？** (1) 如何在没有时序标注的情况下训练模型同时进行视频问答和时序定位？(2) 如何避免模型利用语言偏见绕过视觉内容直接"猜"答案？(3) 如何设计有效的自监督信号来替代缺失的时序标注？
...
**核心idea一句话**：构建对偶的双向推理路径并用循环一致性约束提供时序定位的自监督信号，实现弱监督下的grounded VQA。
