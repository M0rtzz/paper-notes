...
**本文要解决什么？** (1) 如何在避免大量梯度回传的前提下有效实现 CLIP 与 FSAR 的领域对齐？(2) 如何充分利用 CLIP 多阶段特征来提升动作识别的判别力？(3) 如何设计一个既高效又有效的 FSAR 框架？
...
**核心idea一句话**：冻结 CLIP 编码器并缓存多阶段特征，在特征提取之后通过轻量级后推理模块进行多层级（全局、patch、帧）时空推理和匹配，实现高效的小样本动作识别。
