...
description: >-
  [ECCV 2024][LLM 其他][零样本异常检测] 在CLIP中同时引入静态（全局共享）和动态（逐图生成）两种可学习提示，用辅助异常检测数据训练后，在14个工业+医学异常检测数据集上实现零样本SOTA，核心在于"任务级+实例级"双层自适应的混合提示设计。 零样本异常检测（ZSAD）要求模型在从未见过的类别上直接定位异常…
tags:
  - "ECCV 2024"
  - "LLM 其他"
  - "零样本异常检测"
