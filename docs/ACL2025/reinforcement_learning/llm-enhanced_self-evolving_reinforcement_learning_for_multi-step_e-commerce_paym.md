...
description: >-
  [ACL 2025][欺诈检测] 将电商支付欺诈检测建模为多步 MDP，用 LLM（Mixtral/LLaMA/Gemma）通过进化算法自动生成和优化 RL 奖励函数，在 eBay 真实交易数据上比人工设计奖励函数和传统 SL 基线显著提升 dollar-wise precision。
tags:
  - ACL 2025
  - 欺诈检测
  - 强化学习
...
**本文要解决什么？** 如何自动化 RL 奖励函数的设计和优化？

**切入角度**：利用 LLM 的推理和代码生成能力，通过进化算法迭代优化奖励函数代码。

**核心idea一句话**：LLM 作为 RL 奖励函数的自动设计器，通过进化循环根据 agent 表现反馈不断改进奖励。
