...
**领域现状**：词汇语义变迁检测用通时语料嵌入估计词变化程度，但只给词级分数。
**现有痛点**：无法解释**哪些具体用法**变化了；距离度量不区分义项增减；词义扩展/缩小无法统一处理。
**核心矛盾**：如何在保持词级检测能力的同时提供实例级细粒度解释？
**切入角度**：UOT 允许质量创建/销毁——对应新义项出现/旧义项消亡。
**核心 idea**：SUS(x) = 该实例在 UOT 传输计划中的盈余或赤字——正值=义项增加，负值=减少。
