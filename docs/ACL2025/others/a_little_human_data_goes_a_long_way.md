...
**本文要解决什么？** 系统性地量化合成数据替代人工标注数据的极限，特别是在事实验证(FV)和证据型问答(QA)任务中，回答"多少比例的合成数据是安全的"以及"少量人工数据价值几何"。

**切入角度**：固定训练集大小，逐步增加合成数据占比（从0%到100%），在8个不同领域的FV和QA数据集上进行受控实验。

**核心idea一句话**：即使无法大规模人工标注，保留极小比例（2.5%~10%）的人工数据也能带来不成比例的巨大性能提升，合成数据无法完全替代人工数据。
