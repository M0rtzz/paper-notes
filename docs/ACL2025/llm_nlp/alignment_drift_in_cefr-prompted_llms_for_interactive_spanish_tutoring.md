...
**本文要解决什么？** 系统性评估 CEFR-based system prompting 在多轮西班牙语教学对话中约束 LLM 输出难度的有效性和持久性。
**切入角度**：用 LLM 同时扮演"教师"和"学生"（独立聊天记录），模拟完整的多轮对话，避免人类实验的高成本，提供可规模化的低成本评估方法。
**核心idea一句话**：CEFR prompting 对 LLM 的约束效果随对话轮次增加而衰减（alignment drift），仅靠提示工程不足以构建可靠的自适应语言教学系统。
