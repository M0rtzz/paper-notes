...
**本文要解决什么？** 1）如何为 CAD 数据集高效生成高质量的文本标注；2）如何从文本描述准确生成 CAD 建模命令序列；3）如何利用 LLM 的推理能力纠正和提升生成序列质量。

**切入角度**：将精调的小型 Transformer 模型（TCADGen）与 LLM（CADLLM）结合——小模型负责初始序列生成和置信度评估，大模型利用置信度信息聚焦于低置信度命令的纠错优化，而非从零重新生成。

**核心idea一句话**：用双通道 Transformer 生成初始 CAD 序列及逐命令置信度，再用微调的 LLM 根据置信度信息针对性纠错，实现文本到高精度 CAD 模型的自动生成。
