...
description: >-
  [ACL 2025][LLM Agent][task-oriented dialog] 提出一个综合评估框架来量化 LLM agent 与人类专家在任务导向对话中的"行为差距"（dialog acts、工具使用、知识利用三个维度），发现行为差距随任务复杂度增加显著扩大（相关系数 0.963），缩小行为差距可平均提升 24.3% 性能。
tags:
  - ACL 2025
  - LLM Agent
...
## 研究背景与动机
**领域现状**：LLM agent 在任务导向对话系统（TODS）中广泛应用，但 zero-shot 性能与人类专家相比仍有显著差距。
**现有痛点**：之前的工作记录了性能差距的存在，但**行为层面的原因**（LLM 在"怎么做"上与人类有何不同）几乎没有被系统研究。仅有零星工作关注了对话中的 grounding 差异。
**核心矛盾**：我们知道 LLM agent 表现不好，但不知道为什么——是选错了 dialog act？滥用了工具？还是错误地使用了知识？
**本文要解决什么？** 系统化地量化 LLM agent 与人类在对话行为上的差异，分析这些差异如何影响性能，以及是否缩小差距就能提升性能。
**切入角度**：在 teacher-forcing 设置下（避免用户模拟器引入的噪声），从 dialog acts、tool usage、knowledge usage 三个维度评估行为对齐度。
**核心idea一句话**：LLM agent 的性能问题本质上是行为问题——系统量化行为差距后发现，差距与性能退化高度相关，且可通过缩小差距直接改善。
