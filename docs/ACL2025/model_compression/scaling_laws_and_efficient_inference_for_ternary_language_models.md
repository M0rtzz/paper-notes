...
**本文要解决什么？** 三个子问题：（1）建立 TriLM 的 Scaling Law 并训练强开源模型族；（2）设计三值权重的高效存储方案（逼近信息论最优的 1.585 bit/weight）；（3）开发 GPU 推理内核实现端到端加速。
...
**核心 idea 一句话**：TriLM 的 Scaling Law 指数显示"多喂数据"远比"加大模型"更有效，基于此训练 1.2T token 的 Spectra-1.1 模型族，配合专用 TriRun GPU 内核实现端到端 5 倍推理加速。
