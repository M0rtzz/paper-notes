...
description: >-
  [ACL 2025 (Short)][NLP理解][方面级情感分析] 提出 Dynamic Order Template（DOT）方法用于生成式方面级情感分析——为每个实例动态创建最优的预测模板顺序（只含必要的视角），在 ASQP 和 ACOS 数据集上提升 F1 的同时显著减少推理时间。
tags:
  - ACL 2025 (Short)
  - NLP理解
  - 方面级情感分析
...
## 研究背景与动机
**领域现状**：方面级情感分析（ABSA）评估文本中对特定方面的情感，输出结构化的情感元组（方面、类别、情感、观点）。生成式方法将其建模为文本到文本的生成任务，使用模板指导生成顺序。
**现有痛点**：(a) 静态模板（如固定顺序 "方面→观点→情感→类别"）不能捕获元素间的动态依赖；(b) 多视角提示法（Multi-view prompting）用多种模板分别预测再组装，虽然提升了准确率但推理效率低（需要多次推理）且存在分布外错误。
**核心矛盾**：多模板提升质量但推理成本成倍增加，单模板效率高但质量不够——能否兼顾？
**本文要解决什么？** 动态为每个输入实例选择最合适的单一模板顺序，既保留多视角的优势又只推理一次。
**切入角度**：训练一个模板选择器，根据输入文本的特性预测最优的元素预测顺序。
**核心idea一句话**：不用固定模板也不用所有模板——为每个实例动态选择最优的单一模板。
