...
**本文要解决什么？** (1) 系统评估 LLM 生成不同强度 AAE 文本的能力 (2) 比较文本和语音模态下 AAE 对用户体验的影响 (3) 探索非裔口音与方言强度的最优组合

**切入角度**：将方言表达和回复生成解耦（先生成 SAE 回复再翻译为 AAE），控制方言强度为 Low/Medium/High 三档，同时引入 F5-TTS 生成非裔口音语音，文本和语音双通道评估

**核心 idea 一句话**：通过解耦方言翻译和回复生成，系统比较文本 vs 语音模态下 AAE 对真实使用者的影响，发现口音比方言更能有效提升个性化效果。
