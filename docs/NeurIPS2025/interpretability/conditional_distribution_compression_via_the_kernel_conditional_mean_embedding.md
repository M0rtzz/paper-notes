...
description: >-
  [NeurIPS 2025][可解释性][distribution compression] 首次提出针对条件分布：（而非联合分布）的压缩算法，利用核条件均值嵌入（KCME）定义新度量 AMCMD，并设计线性时间算法 ACKIP 构建保留条件分布统计特性的压缩数据集。 领域现状：现有分布压缩方法（如 Kernel Herdi…
tags:
  - "NeurIPS 2025"
  - "可解释性"
