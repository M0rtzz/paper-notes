...
description: >-
  [NeurIPS 2025][模型压缩][video-understanding] 提出 DyToK，一种无需训练的视频 token 动态压缩方法，利用 VLLM 深层注意力中固有的 query 条件关键帧先验，为不同帧自适应分配 token 预算，实现即插即用式的效率-精度最优权衡。 1. 长视频计算瓶颈：Video LL…
tags:
  - "NeurIPS 2025"
  - "模型压缩"
  - "video-understanding"
