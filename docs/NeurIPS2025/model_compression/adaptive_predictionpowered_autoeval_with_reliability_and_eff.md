...
description: >-
  [NeurIPS 2025][模型压缩][LLM evaluation] 提出R-AutoEval+，通过testing-by-betting框架自适应调整对合成数据（LLM评判器）的依赖权重ρ，首次同时保证有限样本可靠性和可证明的采样效率改善，在LLM量化选择、prompt设计和推理预算分配三个任务上验证有效性。
tags:
  - NeurIPS 2025
  - 模型压缩
  - LLM evaluation
