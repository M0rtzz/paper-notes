...
description: >-
  [NeurIPS 2025][理论][自蒸馏] 使用统计物理的replica方法严格分析噪声高斯混合模型上多阶段自蒸馏的效果，揭示硬伪标签的去噪效应是性能提升主因，提出早停和偏置固定两个实用启发式策略，并在CIFAR-10上验证理论发现。
tags:
  - NeurIPS 2025
  - 理论分析
  - 自蒸馏
...
**做什么**：推导每个自蒸馏阶段的分类器权重向量 $w_k$ 与真实方向 $\mu$ 的对齐程度的精确渐近公式。
