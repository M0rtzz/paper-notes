...
**本文要解决什么？** 构建一个客观、可扩展、面向前沿研究问题的agent benchmark，评估LLM agent能否提出并实现真正有效的新方法。

**切入角度**：ML会议竞赛天然具备评估两面性——开放性问题要求创新，公开leaderboard提供客观比较基准。直接利用这些竞赛作为benchmark任务。

**核心idea一句话**：把ML会议竞赛重构为agent-agnostic的标准化环境，用baseline到人类冠军方案的相对进步作为客观度量，同时分析LLM主观评分与客观表现的相关性。
