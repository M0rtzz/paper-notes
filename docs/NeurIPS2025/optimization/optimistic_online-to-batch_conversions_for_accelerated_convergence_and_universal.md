...
**本文要解决什么？** (a) 简化算法设计——让非乐观的在线算法也能实现加速；(b) 首次实现强凸光滑的 O2B 最优收敛；(c) 在保持单次梯度查询的前提下实现通用性。
**切入角度**：乐观性不一定要由在线算法提供，可以通过转换机制本身实现"look-ahead"——评估梯度在提前一步的位置 $\tilde{x}_t$ 而非当前位置 $\bar{x}_t$。
**核心 idea 一句话**：将乐观性从在线算法移到 O2B 转换机制中（look-ahead regret），使简单 OGD 达到加速收敛，并统一处理强凸和通用设置。
