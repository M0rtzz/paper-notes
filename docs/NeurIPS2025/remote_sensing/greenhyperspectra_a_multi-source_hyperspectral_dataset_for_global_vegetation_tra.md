...
**本文要解决什么？** (1) 构建跨传感器、跨生态系统的大规模高光谱预训练数据集；(2) 建立半/自监督学习在多输出回归任务上的基准方法框架；(3) 验证预训练方法在标签稀缺和分布外场景中的优势。

**切入角度**：利用来自不同平台（近端/航空/卫星）、不同年份（1992-2024）、不同生态系统的14万+无标签高光谱样本进行预训练，学习跨域鲁棒的光谱表征。

**核心idea一句话**：构建多源异质高光谱预训练数据集GreenHyperSpectra，结合MAE等自监督方法解决植物性状预测中"标签稀缺+域偏移"的双重挑战。
