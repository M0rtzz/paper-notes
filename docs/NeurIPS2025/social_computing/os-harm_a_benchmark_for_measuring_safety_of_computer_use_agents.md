...
**本文要解决什么？** (a) 构建覆盖操作系统级多应用的 Agent 安全 benchmark (b) 设计自动化 LLM judge 替代手写评分函数 (c) 系统评估前沿模型在三类安全风险下的表现。
**切入角度**：基于 OSWorld 这一成熟的开源环境，将安全任务无缝集成到已有的 Agent 评测框架中，确保任务在真实 VM 中执行而非模拟调用。
**核心idea一句话**：在真实操作系统环境中构建三维度（恶意使用 / 注入攻击 / 模型失误）安全 benchmark，用 LLM judge 自动评估 Agent 的安全性与任务完成度。
