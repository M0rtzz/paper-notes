...
**本文要解决什么？** LLM 在资源分配中是否与人类价值观对齐？行为受哪些公平公理支配？错位根源是什么？

**切入角度**：采用 Herreiner & Puppe (2010) 的经典人类实验数据集，设计不可分物品（有/无金钱）的分配实例，构造公平-效率之间的 tradeoff 场景。

**核心idea一句话**：人类优先追求平等性 (EQ)，而 LLM 优先追求经济效率 (PO/USW) 和无嫉妒性 (EF)——但当 LLM 从预设选项中选择（而非自行生成）时，GPT-4o 和 Claude 能正确识别最公平方案。
