...
**定性为主**：大多依赖定性描述或综合指数，掩盖了不同政策领域的关键差异

**缺乏主题区分**：未能区分 Mitigation（减缓）、Adaptation（适应）、Disaster Risk Management（灾害管理）和 Loss & Damage（损失与损害）之间的差异

**政策-发展关联缺失**：虽然政策数量不断增长，但对其与 GDP、GNI、FDI 等发展结果的关联研究有限

**可比性不足**：跨国比较缺乏标准化的量化框架
