...
**本文要解决什么？**: 为 mean 聚合的 GNN 建立 sound 逻辑规则的理论基础，并提供实际可用的规则提取方法。
**切入角度**: 限制权重为非负（与 sum/max-GNN 工作一致），将非单调性来源隔离到聚合函数本身，从而简化分析。
**核心idea一句话**: MAGNN 能表达超越 FOL 的逻辑函数，但其 sound 单调规则形式非常受限——所有 sound ELUQ 规则都可被一组极简规则子覆盖（subsume），并可用有限搜索空间枚举验证。
