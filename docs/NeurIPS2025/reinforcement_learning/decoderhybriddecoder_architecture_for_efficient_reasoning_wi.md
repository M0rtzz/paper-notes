...
**本文要解决什么？** 如何在不损失长上下文检索能力的前提下，减少 cross-decoder 中注意力层的内存 I/O 开销？
...
**核心idea一句话**：用 GMU 替换一半 cross-attention 层，将解码时的内存 I/O 从线性 $O(d_{kv} \cdot N)$ 降为常数 $O(d_h)$，同时通过门控实现对先前 token mixing 的细粒度重新加权。
