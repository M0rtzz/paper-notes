...
**本文要解决什么**：(a) 不依赖 $T$ 先验实现最优 $\sqrt{P_T}$ 依赖；(b) 将 reduction 推广到任意凸损失；(c) 利用损失曲率获得 $\mathcal{O}(\|u\|_{\mathcal{H}}^2 + d_{\text{eff}}(\lambda)\ln T)$ 的 bound。
**切入角度**：观察到与任意比较器序列竞争等价于与一个固定函数 $u(\cdot)$ 竞争，只要 $u(t) = u_t$。将这个函数嵌入 RKHS 中，就把动态遗憾降维为函数空间中的静态遗憾。
**核心idea一句话**：动态遗憾 = RKHS 中的核化静态遗憾，通过设计合适的核函数控制 RKHS 范数与路径长度的关系。
