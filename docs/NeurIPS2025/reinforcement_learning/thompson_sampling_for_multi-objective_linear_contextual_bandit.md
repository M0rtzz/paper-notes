...
**领域现状**：多目标多臂 Bandit（MOMAB）中，Pareto 最优比标量化方法更通用。但所有现有 Pareto regret 分析都基于 UCB 算法
**现有痛点**：TS 在单目标中实证表现常优于 UCB，但多目标 TS 的理论分析存在根本困难——需跨多个目标协调随机采样的乐观性
**核心矛盾**：单目标 TS 的单次采样有常数概率乐观，但多目标需要所有目标同时乐观，概率随目标数 $L$ 指数下降
**核心 idea**：通过每个目标多次采样（$M = O(\log L)$ 次）并取最大值，避免乐观概率的指数衰减
