...
## 研究背景与动机
**领域现状**：3DGS 在密集视角下表现优异，但稀疏视角下新视角会出现外观伪影（颜色异常值）
**现有痛点**：现有方法主要通过几何正则化（单目深度约束等）改善几何，但很少研究外观伪影
**核心发现**：多个颜色不同的高斯体过度协同合作来拟合有限的训练像素，忽略了场景的真实外观分布。这种"协同适应"类似于神经网络中的过拟合
**核心 idea**：借鉴 Dropout 的思想——随机丢弃部分高斯体迫使每个高斯体独立地编码正确外观
