...
**本文要解决什么？** 在不修改模型权重、不需要训练的前提下，利用投机解码中草稿模型的注意力分布来指导验证模型的 KV 缓存动态剪枝，实现内容感知的稀疏注意力，同时保持输出质量。
**核心 idea 一句话**：草稿模型的注意力权重 → KL 散度映射到验证模型层 → 免排序 top-p 选择重要 token → 动态剪枝验证模型 KV 缓存 = 零训练成本的内容感知稀疏注意力。
