...
description: >-
  [NeurIPS 2025][NLP理解][弱到强泛化] 发现朴素的弱到强泛化 (weak-to-strong generalization) 在分布偏移下会失败（强模型表现甚至不如弱监督者），提出 RAVEN 框架通过动态学习弱模型的最优组合权重来实现鲁棒的弱到强泛化，在 OOD 任务上超越基线 30%+。
tags:
  - NeurIPS 2025
  - NLP理解
  - 弱到强泛化
...
**代码**: 待确认  
**领域**: NLP理解 / 模型对齐 / 弱到强泛化  
**关键词**: 弱到强泛化, 分布偏移, RAVEN, 超人模型监督, 对齐
...
1. **多弱模型组合学习**：
    ...
    - 功能：维护一组弱模型 $\{w_1, w_2, \ldots, w_K\}$，每个弱模型在不同分布/域上各有专长
