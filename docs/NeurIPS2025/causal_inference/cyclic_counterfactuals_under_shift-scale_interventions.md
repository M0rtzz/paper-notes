...
**本文要解决什么？** 在循环因果模型中，shift-scale干预下的反事实分布何时存在且唯一？这类干预是否具有代数稳定性（可组合性）？反事实结果的分布有多集中？

**切入角度**：借用动力系统理论中的收缩映射原理（Banach不动点定理），为循环SCM的唯一可解性提供充分条件，然后证明shift-scale干预保持收缩性。

**核心idea一句话**：用全局收缩条件统一处理循环SCM中soft intervention的反事实推理，将唯一可解性从DAG推广到满足收缩条件的循环图。
