...
description: >-
  [NeurIPS 2025][多模态][MLLM可解释性] 发现 MLLM 中视觉相关的功能解码分布在特定的窄层中（视觉功能层/VFL），不同功能呈现一致的层级模式（识别→计数→定位→OCR），并据此设计了 VFL-LoRA 和 VFL-select 方法。
tags:
  - NeurIPS 2025
  - 多模态
  - MLLM可解释性
...
## 研究背景与动机
**领域现状**：MLLM 的内部视觉处理机制是"黑箱"——我们不知道不同层对不同视觉任务的具体贡献
**现有痛点**：现有研究只给出粗略结论（"浅层提取视觉特征，深层做推理"），缺乏精细的功能定位框架
**核心创新**：提出 Visual Token Swapping 框架——在指定层交换两张图的视觉 KV cache，观察输出变化率来定位功能层
