...
**本文要解决什么？**: 精确回答"两层过参数化 ReLU 网络的稳定极小值在高维非插值情形下的泛化表现如何"这一根本问题。

**切入角度**: 建立稳定极小值的函数空间刻画（加权变分范数），然后分别推导泛化间隙和 MSE 的上下界，并通过基于球面覆盖的新型极小极大下界构造揭示几何机制。

**核心idea一句话**: 高维球面上存在指数多的"方向帽"(spherical caps)，使得 ReLU 神经元可以仅激活极少数据点却保持大权重，从而"欺骗"平坦性标准——这就是 neural shattering，它导致稳定极小值在高维下不可避免地遭遇维度灾难。
