...
**本文要解决什么？** 提供一个完全开放（数据+代码+权重）、性价比高、多尺度的视觉编码器家族，使其在多模态学习场景下达到或超越闭源水平。
**切入角度**：基于最新的开源进展——CLIPS 训练框架（多正样本对比+文本生成器）和 Recap-DataComp-1B（重新标注的十亿级数据集），系统性分析关键设计选择。
**核心idea一句话**：站在 CLIPS+Recap-DataComp-1B 肩上，系统调优训练策略，发布25+个完全开源视觉编码器checkpoint，全面匹配/超越 OpenAI CLIP。
