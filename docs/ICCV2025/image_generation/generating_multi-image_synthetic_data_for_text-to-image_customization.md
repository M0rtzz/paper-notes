...
description: >-
  [ICCV 2025][图像生成][文本到图像定制化] 提出 SynCD（合成定制数据集）及其生成管线，利用共享注意力和 3D 资产先验合成多图一致性对象数据集，训练的编码器模型在无需测试时优化的情况下超越现有编码器方法。 文本到图像模型的定制化（Customization）旨在学习用户提供的参考对象，并通过文本提示生成该对…
tags:
  - "ICCV 2025"
  - "图像生成"
  - "文本到图像定制化"
