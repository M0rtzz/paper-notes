...
**本文要解决什么？** (a) 将多步扩散渲染加速为单步推理；(b) 建立逆渲染和前向渲染之间的循环一致性；(c) 扩展到室内外多域场景；(d) 以免训练方式实现视频分解。
...
**核心 idea 一句话**：将 RGB↔X 微调为两个单步扩散模型（50× 加速），通过循环一致性损失联合训练使双向渲染互相正则化，并利用循环结构引入无标注真实数据实现跨域泛化。
