...
**本文要解决什么？** (a) 如何在黑盒设置下实现有效的梯度驱动对抗 prompt 学习？(b) 如何解决安全机制返回黑图导致的梯度消失问题？

**切入角度**：利用目标 prompt 中蕴含的敏感信息作为语义引导，结合一个无安全机制的辅助模型生成目标图像，通过多模态（文本-图像、图像-图像）相似度构建可计算梯度的训练目标。

**核心 idea 一句话**：通过敏感知识编码保留目标 prompt 的语义意图，结合基于CLIP相似度的多模态损失和改进的零阶梯度优化，在黑盒设置下训练 prompt encoder 生成能绕过双重安全机制的对抗性 prompt。
