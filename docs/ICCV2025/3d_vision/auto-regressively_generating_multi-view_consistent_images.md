...
description: >-
  [ICCV 2025][3D视觉][多视图生成] 提出 MV-AR，首次将自回归模型引入多视图图像生成，利用所有先前视图作为条件逐步生成后续视图，配合统一的多模态条件注入模块和 Shuffle View 数据增强，在文本/图像/形状条件下均达到与扩散模型可比的一致性。 从文本、参考图像或几何形状生成多视图一致图像是3D内容创…
tags:
  - "ICCV 2025"
  - "3D视觉"
  - "多视图生成"
