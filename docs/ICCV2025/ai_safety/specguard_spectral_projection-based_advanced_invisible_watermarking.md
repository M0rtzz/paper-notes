...
**本文要解决什么**：设计一种同时在不可见性和鲁棒性上超越 SOTA 的水印方法，特别是要抵御三类攻击：畸变（rotation、crop、noise 等）、图像再生成（扩散模型重建）、对抗攻击。

**切入角度**：将水印嵌入到隐藏卷积层的频谱域（而非直接的空间域或简单的频域），通过"小波投影→高频子带→FFT 频谱投影"的层叠变换，让水印信息深度隐藏在高频频谱分量中。

**核心 idea 一句话**：在小波分解的高频子带上做 FFT 频谱投影，将水印嵌入到频谱域的高频区域，配合可学习阈值（基于 Parseval 定理）实现高精度比特提取。
