...
**本文要解决什么？** (a) 理论上回答"什么条件下替换终端分布能改善生成"；(b) 设计联合均值-协方差估计器；(c) 提出统一框架适用于扩散模型和流匹配。
**切入角度**：条件白化——用估计的条件均值去中心化 + 用估计的条件协方差的逆平方根标准化，等价于对数据做线性变换使其更接近标准高斯。
**核心idea一句话**：将扩散/流匹配的终端分布从 $\mathcal{N}(0,I)$ 替换为 $\mathcal{N}(\hat{\mu}_{X|C}, \hat{\Sigma}_{X|C})$，通过条件白化变换让去噪网络只需学习残差。
