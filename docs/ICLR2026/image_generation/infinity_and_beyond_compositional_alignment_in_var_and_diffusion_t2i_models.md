...
本文不提出新模型，而是搭建一个统一的评测协议，把三种生成范式放到同一张桌子上比组合对齐能力。整个流程是：先准备覆盖颜色/属性/空间/计数/复杂组合等多维度的同一批 prompt，喂给一个横跨三种架构、参数从 0.6B 到 12B 的模型谱系——扩散类（SDXL、扩散 Transformer PixArt-$\alpha$）、DiT 类（Flux-Dev / Flux-Schnell）以及代表 VAR 路线的 Infinity-2B / Infinity-8B；每个模型对每条 prompt 用多个随机种子各生成一批图以平掉随机性；最后用 T2I-CompBench++ 和 GenEval 两个验证机制根本不同的基准分别打分，得到一张可横向比较、且经两套机制交叉确认的组合对齐画像。

```mermaid
%%{init: {'flowchart': {'rankSpacing': 24, 'nodeSpacing': 28, 'padding': 6, 'subGraphTitleMargin': {'top': 8, 'bottom': 16}, 'wrappingWidth': 400}}}%%
flowchart TD
    P["组合 prompt 集<br/>覆盖 8+7 个对齐维度"] --> M
    subgraph M["跨架构跨规模模型谱系"]
        direction TB
        M1["扩散 UNet/Transformer<br/>SDXL · PixArt-α"]
        M2["DiT：Flux-Dev · Flux-Schnell"]
        M3["VAR：Infinity-2B · Infinity-8B"]
    end
    M --> G["多种子平均生成<br/>每 prompt × 4 随机种子"]
    subgraph V["双基准互补验证"]
        direction TB
        B1["T2I-CompBench++<br/>检测器驱动"]
        B2["GenEval<br/>规则约束"]
    end
    G --> V
    V --> R["种子平均分数<br/>跨架构组合对齐画像"]
```

### 关键设计

**1. 跨架构跨规模模型谱系：把"架构差异"和"规模效应"拆开看**

要回答"VAR 到底强不强"，最怕的就是把架构优势和参数优势搅在一起。本文因此精心铺了一条谱系：参数量从 0.6B（PixArt-$\alpha$）一直拉到 12B（Flux），横跨扩散 UNet、扩散 Transformer、DiT、VAR 四类骨干，保证比较不被某一类模型主导。其中 Infinity-2B 与 Infinity-8B 构成 VAR 路线内部的对照，专门观察同架构下放大规模的收益；Flux-Dev 与 Flux-Schnell 则构成质量-速度的对照，用来检查蒸馏加速是否牺牲组合正确性。有了这套对照设计，"是 VAR 架构本身好，还是单纯参数多"这类混淆因素才能被分离出来——后文也正是靠 Infinity-2B 以 2B 参数追平 12B Flux，证明确有架构层面的增益。

**2. 多种子平均生成：抑制随机性带来的排名抖动**

生成模型对随机种子敏感，单张图的好坏不能代表模型能力，否则偶然的好运会被误读成架构优势。评测协议因此对每条 prompt 用 4 个独立随机种子各生成一批图：T2I-CompBench++ 报告 4 个种子的平均分，并在附录给出标准差，用来判断模型间差距是否真的超出噪声范围；GenEval 也遵循官方协议每条 prompt 生成 4 个样本，但因其只输出聚合分数、不暴露样本级方差，故只报告聚合值。这样得到的排名才是稳定、可复现的。

**3. 双基准互补验证：用两套不同机制交叉确认结论**

单一基准的打分逻辑可能带有系统性偏差，只靠它得出"VAR 更强"很难服众。本文因此同时采用两个验证范式根本不同的基准互为 sanity check：T2I-CompBench++ 由**检测器驱动**，先在生成图上检测物体再核对属性与关系，覆盖颜色、纹理、形状绑定、非空间关系、2D/3D 空间关系、计数和复杂组合共 8 个维度，偏"看图找证据"；GenEval 则用**规则化约束**判定，覆盖单物体、双物体、计数、颜色、位置、颜色属性和总体共 7 个维度，偏"按规则卡条件"。只有当两套机制给出一致排序时，结论才被认为可靠——后文 Infinity-8B 在两个基准上同时登顶，正是这层交叉确认带来的说服力。
