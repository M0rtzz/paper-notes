...
**本文要解决什么？** 提供首个统一的 VAR vs 扩散模型组合对齐基准评测，覆盖颜色/纹理/形状绑定、空间关系、计数和复杂多属性组合。
**切入角度**：使用两个互补基准——T2I-CompBench++（检测器驱动验证）和 GenEval（规则化约束），覆盖 8 个评估维度，对 6 个代表性 T2I 模型进行全面评测。
**核心 idea 一句话**：VAR 模型（特别是 Infinity-8B）在组合对齐上系统性地优于扩散模型，可能因为 next-scale 自回归生成天然地在每个阶段条件化于已生成的视觉结构。
