...
description: >-
  [ICLR 2026][优化/理论][Multi-Index模型] 证明在通用非退化假设下，标准两层神经网络通过分层梯度下降可以用 $\tilde{O}(d)$ 样本和 $\tilde{O}(d^2)$ 时间学习通用高斯 Multi-Index 模型 $f(\bm{x})=g(\bm{U}\bm{x})$，样本和时间复杂度都达到信息论最优，首次证明神经网络可以高效学习层次化函数。
tags:
  - "ICLR 2026"
  - "优化/理论"
  - "Multi-Index模型"
...
### 整体框架
目标是让一个普通的两层网络 $f_{\Theta}(\bm{x}) = \sum_j a_j \sigma(\bm{w}_j^T \bm{x} + b_j)$，只用梯度下降就把 Multi-Index 模型 $f^*(\bm{x})=g^*(\bm{U}\bm{x})$ 隐藏的 $r$ 维子空间 $\bm{U}$ 学出来，且样本量逼近信息论下界 $\tilde{O}(d)$。本文不用花哨算法，而是把训练拆成前后两层、分两个阶段顺序进行：**阶段1**先在数据集 $\mathcal{D}_1$ 上训练内层权重 $\bm{W}$（$T_1$ 步），让神经元的方向自动对齐到隐子空间——这一阶段隐式地在做"带早停的幂迭代"，是全文的核心；**阶段2**再重新初始化偏置 $\bm{b}$，在独立的数据集 $\mathcal{D}_2$ 上训练外层权重 $\bm{a}$（$T_2$ 步），这是一个凸问题、标准 GD 即可收敛到最优。下面三个关键设计都围绕阶段 1 的内层动力学展开。

### 关键设计

**1. 隐式幂迭代机制：把内层梯度下降还原成对局部 Hessian 的幂方法**

为什么神经网络靠纯梯度下降就能找到隐子空间？本文证明，阶段 1 前 $T_1$ 步内层更新，本质上是在对局部 Hessian $\hat{\Sigma}_\ell = \frac{1}{n}\sum_i \ell_i \bm{x}_i\bm{x}_i^T$ 做幂迭代。关键在小初始化（$\epsilon_0 \to 0$）：此时激活函数被泰勒展开近似成二次（$\sigma''(0)=1$），各神经元的梯度更新几乎解耦独立，每一步退化为

$$\bm{w}_j^{(t+1)} \approx \hat{\Sigma}_\ell\, \bm{w}_j^{(t)},$$

也就是标准幂方法的迭代式。而 $\hat{\Sigma}_\ell$ 的谱恰好把信号和噪声分开：大特征值对应真正的隐子空间方向，小特征值对应 $n$ 有限带来的采样噪声。幂迭代天然放大大特征值、压制小特征值，于是信号方向被指数级放大、噪声被衰减——但前提是迭代必须在正确的步数停下来，这就引出第 2 点。

**2. 关键停时条件：内层训练步数 $T_1$ 必须不多不少**

幂迭代放大信号是把双刃剑：停早了和停晚了都会出问题。$T_1$ 太小，噪声方向还没充分衰减（在 $n \sim d$ 这种最难的样本量下尤其明显），子空间估计被噪声污染；$T_1$ 太大，所有神经元都被拉向最大特征值方向，结果只剩一个方向、丢掉了 $r$ 维子空间里其余的非主导方向。最优的 $T_1$ 卡在一个窗口里：让信号的全部 $r$ 个方向都被足够多的神经元覆盖，同时把噪声压到可忽略。一个反直觉的推论是——最优学习需要的第一层步数**不是** $O(1)$ 而是发散的（$T_1 \to \infty$），这与基于 DMFT / 状态演化、只跑 $O(1)$ 步的分析框架根本对不上。

**3. 通用损失函数 = 数据预处理：用损失的选择去调 $\Sigma_\ell$ 的谱**

本文把"选哪个损失函数"重新解读成"做哪种特征预处理"。原因是局部 Hessian 的谱结构由损失的一阶导 $\ell'(0, y)$ 决定：换损失就换了 $\Sigma_\ell$ 的特征结构，也就换了幂迭代能放大哪些方向。对生成指数为 2 的 Multi-Index 模型，存在某个损失使 $\Sigma_\ell$ 非退化（Assumption 5），从而所有信号方向都能被激活。这把损失函数从单纯的"优化目标"提升为"特征提取的预处理器"，也因此把适用范围从平方损失拓展到 $\ell^1$、Huber 等更一般的目标函数。
