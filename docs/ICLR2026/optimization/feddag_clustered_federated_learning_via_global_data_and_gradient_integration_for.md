...
**本文要解决什么？** 如何综合利用数据和梯度信息动态聚类客户端，同时允许集群间的表征共享？

**切入角度**：将相似度计算细化到类别级（class-wise），为数据和梯度信号自动学习权重，并用双编码器架构实现跨集群特征迁移。

**核心idea一句话**：通过类别级加权融合数据和梯度相似度进行更精准的客户端聚类，并用双编码器架构在保持集群特化的同时实现跨集群知识共享。
