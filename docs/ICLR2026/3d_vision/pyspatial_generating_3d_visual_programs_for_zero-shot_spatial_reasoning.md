...
description: >-
  [ICLR 2026][3D视觉][视觉编程] 提出pySpatial——视觉编程框架让MLLM通过生成Python代码调用3D空间工具(重建/相机恢复/新视角渲染)实现零样本3D空间推理：将2D输入转化为可探索的3D场景→MLLM在结构化3D表示上显式推理而非隐式想象,在MindCube上超越GPT-4.1-mini 12.94%,并成功用于真实室内四足机器人导航。
tags:
  - ICLR 2026
  - 3D视觉
  - 视觉编程
...
**领域现状**：MLLM在感知和2D推理上表现好→但3D空间推理(深度/视角变换/多视角关系)仍很弱→接近随机猜测级别。
...
**切入角度**：不让MLLM隐式推理3D→而是显式构建3D场景→让MLLM在3D场景上操作→视觉编程。
