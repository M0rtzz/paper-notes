...
**领域现状**：程序化材质(如Substance Designer)用节点图定义PBR材质→参数化/分辨率无关/可编辑。创建节点图需要专业训练→对新手门槛高。神经程序合成(MatFormer, VLMaterial)尝试自动化。
...
**切入角度**：模拟人类艺术家工作流→在生成过程中提供中间节点的视觉渲染→多模态(视觉+文本)反馈循环。
