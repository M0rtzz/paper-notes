...
**本文要解决什么？** 如何高效精确求解0-1损失线性分类问题？Cover计数定理、Murthy的 $2^D \binom{N}{D}$ 分析、以及Nguyen-Sanner的 $\binom{N}{D}$ 观察这三种看似矛盾的复杂度分析之间的关系？精确解是否真的过拟合？

**切入角度**：从超平面排列和有向拟阵理论出发，利用点-超平面对偶变换建立数据点与线性二分类之间的组合关系和入射关系。

**核心idea一句话**：通过对偶变换将分类问题转化为超平面排列的单元枚举问题，证明只需枚举 $\binom{N}{D}$ 个通过 $D$ 个数据点的超平面即可找到全局最优，并利用增量组合生成器实现高效枚举。
