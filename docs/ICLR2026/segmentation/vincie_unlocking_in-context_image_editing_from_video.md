...
**本文要解决什么？** 是否可以仅从视频数据中学习出有意义的in-context图像编辑模型，无需任何独立图像对。

**切入角度**：视频天然包含丰富的视觉动态变化（物体出入、姿态变化、相机运动），这些隐式地提供了编辑操作的学习信号。

**核心idea一句话**：从原生视频数据中构建交错多模态序列（帧+转换描述+分割掩码），用三个代理任务训练DiT模型学习上下文感知的图像编辑。
...
1. **可扩展的视频数据标注流水线**:
    - 功能：将原生视频转化为适合训练in-context编辑模型的交错多模态序列
...
2. **DiT架构与上下文组合学习**:
    - 功能：基于Diffusion Transformer框架学习上下文条件下的图像生成
