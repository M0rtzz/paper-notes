...
**本文要解决什么？** （1）如何消除开放式情感评估的答案歧义？（2）如何在保持可扩展性的同时覆盖细粒度情感？（3）如何将场景上下文和主观性纳入评估维度？（4）如何以最小人工代价构建大规模评估数据？

**切入角度**：受认知心理学启发，将情感评估从"生成式回答"转为"判断式验证"——让模型判断图像与情感陈述是否匹配，同时设计四个互补维度覆盖从基本情感识别到主观性理解的完整能力谱。

**核心idea一句话**：用"判断情感陈述对错"替代"回答情感是什么"，从根本上消除开放式评估的歧义，同时通过自动化流水线实现开放词汇、多维度、大规模评估。
