...
description: >-
  [ICLR 2026][LLM效率][对抗性评测] 提出SwingArena对抗性评测框架，让LLM交替扮演提交者(生成补丁)和审查者(编写测试)，通过真实CI流水线验证，覆盖C++/Python/Rust/Go四种语言的400个GitHub issue，揭示不同模型在补丁生成vs验证方面的行为差异。
tags:
  - ICLR 2026
  - LLM效率
  - 对抗性评测
...
## 研究背景与动机
**现有代码基准过于静态**：HumanEval/MBPP仅评估短代码片段，SWE-Bench限于Python且只用单元测试，都无法捕捉真实软件开发的迭代协作特性

**忽视完整CI流水线**：真实PR需通过编译、linting、风格检查、回归测试等完整流程，现有基准仅问"代码能否通过单元测试"

**缺乏对抗性交互**：真实开发中reviewer会主动构造corner case挑战补丁质量，单agent静态评测无法暴露这类问题

**长上下文代码检索挑战**：大型代码库中相关信息分散在千行代码和多文件中，需要有效的检索策略

**多语言支持不足**：大多数基准仅支持Python，忽略了C++、Rust、Go等主流语言
