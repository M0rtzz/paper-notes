...
**本文要解决什么？**：如何将任意自回归语言模型**无损地**转换为使用任意子词表的等价模型？进而，如何找到多个模型间的最优公共词表，使之在保证无损性的同时最大化生成效率？

**切入角度**：从概率论出发，将词表缩减形式化为嵌套分词下的分布等价问题。论文证明了只要目标子词表满足覆盖条件，存在唯一的等价 next-token 分布，并给出了递归构造算法。

**核心idea一句话**：通过嵌套分词的精确概率分解，实现从大词表到任意小词表的无损转换，配合最大公共词表（MCV）统一不同模型的输出空间以实现集成。
