...
**本文要解决什么？** (1) 定义和形式化"Theory of Space"这一能力维度；(2) 构建一个可控的评估基准，解耦探索能力和推理能力；(3) 直接探查模型的内部空间信念质量而非仅看任务结果；(4) 测试模型在动态环境中修正空间信念的能力（对标Theory of Mind中的False Belief）。
...
**核心idea一句话**：通过认知地图探查和False Belief范式，直接评估基础模型主动构建和动态修正空间信念的能力。
