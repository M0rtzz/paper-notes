...
**本文要解决什么？** (1) 如何在不重训练模型的情况下提升代码生成的安全性？(2) 如何利用持续演化的社区安全知识来弥补模型静态知识的不足？(3) 如何设计一种可解释、可适应的推理时安全干预机制？

**切入角度**：作者观察到 Stack Overflow 社区十多年来积累了丰富的安全讨论——开发者在评论中指出代码为什么不安全、建议更安全的替代方案。这些人类撰写的解释性知识恰好是 LLM 训练数据中缺失的 "why" 层面的安全推理。

**核心idea一句话**：用 BM25 从 Stack Overflow 安全讨论库中检索与生成代码相似的安全警告，作为推理时上下文引导 LLM 自主修订不安全代码。
