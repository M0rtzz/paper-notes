...
4. **本文要解决什么？** (1) 如何在体素特征中嵌入3D位置信息以纠正投影误差？(2) 如何让NeRF分支更聚焦于前景物体区域？(3) 如何确保不透明度预测在不同视角间保持一致？
5. **切入角度**：从三个互补的角度全面优化NeRF-based检测器——体素构造层面（PEOM）、采样策略层面（DIS）和不透明度预测层面（OOM），三者协同形成端到端网络。
6. **核心idea一句话**：通过嵌入3D位置信息优化体素、双重重要性采样聚焦前景、多视图一致性约束优化不透明度，全面增强NeRF分支的几何感知能力以提升3D检测性能。
