...
- **做什么**: 将高分辨率 3D 场景 TUDF grid 渐进分解为 N 层树，每层 $i$ 包含几何体积 $L_i^s$（TUDF）和潜在特征体积 $H_i^s$。
...
**2. 基于 Patch 的条件扩散**
- **做什么**: 在潜在树每层训练 3D UNet 扩散模型，从几何体积 $L_i$ 预测潜在特征 $H_i$。
...
**3. 大规模场景的 Patch 拼接生成**
- **做什么**: 推理时通过 patch 拼接 + 从粗到细的层级重建生成任意大小场景。
- **核心机制**:
