...
**本文要解决什么？** (1) 如何让高斯点的变形更灵活更个人化；(2) 如何减少表情追踪误差对重建的影响；(3) 如何在未知光照的单目视频下实现真实的重光照。

**切入角度**：作者观察到每个人的面部形状和变形模式都是独特的，通用参数模型无法精确表达，因此为每个高斯点独立学习blendshapes基函数和混合权重。同时利用端到端训练的表情编码器替代独立的预追踪步骤。

**核心idea一句话**：用可学习的逐点blendshapes和LBS替代刚性FLAME绑定，结合端到端表情编码器和物理渲染模型，实现高质量可重光照的头部avatar重建。
