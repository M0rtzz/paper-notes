...
description: >-
  [CVPR 2025][3D视觉][个性化头像] PERSE 从单张肖像照出发，通过合成大规模面部属性编辑视频数据集并训练基于 3DGS 的生成式头像模型，实现了在连续可解耦的潜在空间中对面部属性进行平滑插值编辑，同时保持个体身份一致性。 领域现状：3D 面部头像生成是 AR/VR、数字人、影视制作等领域的核心技术。近年来…
tags:
  - "CVPR 2025"
  - "3D视觉"
  - "个性化头像"
