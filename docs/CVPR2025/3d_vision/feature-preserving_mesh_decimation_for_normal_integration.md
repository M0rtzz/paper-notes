...
**本文要解决什么**: 设计一种各向异性网格简化算法，使三角网格的顶点和边沿着表面脊线/沟槽对齐，在极高压缩率下仍保持几何精度。

**切入角度**: 将 3D 网格简化中的 QEM 推导到 2D 屏幕空间（用法线贴图代替 3D 几何），并利用广义 Delaunay 三角化实现边的各向异性对齐。

**核心 idea 一句话**: 在法线积分之前，用法线贴图驱动的屏幕空间 QEM + 广义 Delaunay 实现各向异性网格预处理，以极少顶点忠实保留几何特征。
...
- **做什么**: 从法线贴图直接推导 QEM，而非依赖已知的 3D 几何。通过法线计算表面切线（Jacobian $J_f$），将 3D 表面积分转化为屏幕坐标积分。
...
- **做什么**: 利用 $\|\cdot\|_M$ 范数扩展标准 Delaunay 三角化到各向异性场景，通过局部四面体判据决定是否翻转边。
...
- **做什么**: 用 quadric 代价 $C_{vw} = \min_{\vec{u}_{vw}} \tilde{Q}_v(\vec{u} - \vec{u}_v) + \tilde{Q}_w(\vec{u} - \vec{u}_w)$ 衡量每次折叠对表面形状的影响，优先折叠代价最小的边。
