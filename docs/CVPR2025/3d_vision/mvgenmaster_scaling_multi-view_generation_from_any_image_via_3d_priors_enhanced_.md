...
4. **本文要解决什么？** 如何构建一个既支持灵活输入（1-多参考视图）又能一次性生成大量新视角的多视图扩散模型，同时保证 3D 一致性和跨域泛化？

5. **切入角度**：引入基于度量深度和相机位姿的几何 warp 作为显式 3D 先验，让扩散模型既"生成"又"重建"。同时构建 160 万场景的大规模数据集来扩展训练。

6. **核心idea一句话**：用度量深度 warp 的 RGB 像素和规范坐标图 (CCM) 作为 3D 先验注入多视图扩散模型，结合 key-rescaling 解决长序列注意力稀释，实现灵活、可扩展、高一致性的多视图生成。
