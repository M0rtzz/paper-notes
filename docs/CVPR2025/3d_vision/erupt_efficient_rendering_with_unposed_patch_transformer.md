...
4. **本文要解决什么？** 设计一个同时支持有位姿和无位姿训练、推理时可直接指定相机位姿、且计算效率提升一个数量级的通用化新视角合成模型。

5. **切入角度**：将编码器中的自注意力和交叉注意力交替使用来区分不同图像的 token，并引入 patch-based 解码替代像素级解码。

6. **核心idea一句话**：通过"patch光线查询+可学习潜在位姿+交替注意力场景Transformer"三位一体的架构设计，同时解决了无位姿训练、直接相机控制和计算效率三个问题。
