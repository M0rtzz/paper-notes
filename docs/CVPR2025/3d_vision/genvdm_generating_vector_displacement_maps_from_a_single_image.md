...
**本文要解决什么？** 如何从单张 RGB 图像生成高质量的 VDM？具体包括：(1) 如何生成能解决遮挡的多视角几何表示；(2) 如何将重建的网格参数化为 VDM 图像格式；(3) 如何构建训练数据集。
...
**核心idea一句话**：用多视角法线生成解决遮挡问题，用神经 SDF 重建网格，再用 MLP 变形场将网格参数化为 VDM 图像，以仅 1200 个训练样本实现了高质量的零样本 VDM 生成。
