...
4. **本文要解决什么？** (1) 如何让 Flow Matching 直接从一种模态演化到另一种？(2) 如何解决源目标形状不匹配问题？(3) 如何在没有显式条件机制的情况下启用 CFG？
5. **切入角度**：由于同一数据点的不同模态之间存在信息冗余，条件模态的数据分布天然与目标分布相关。直接从相关分布出发应该比从噪声出发更容易学习——路径更短更高效。
6. **核心idea一句话**：用变分编码器将源模态数据压缩为与目标模态相同形状的正则化潜在空间，然后用 Flow Matching 直接从源模态潜空间演化到目标模态空间，无需噪声和条件机制。
