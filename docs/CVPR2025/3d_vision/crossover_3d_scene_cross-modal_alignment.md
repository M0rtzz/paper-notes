...
description: >-
  [CVPR 2025][3D视觉][跨模态对齐] 提出CrossOver框架，通过维度特定编码器和三阶段训练管线，在不要求完整模态配对的条件下，学习RGB图像、点云、CAD模型、平面图和文本描述的统一场景级跨模态嵌入空间，支持灵活的跨模态检索和定位。 - 现有多模态3D理解方法（ULIP、PointBind等）聚焦于物体级别…
tags:
  - "CVPR 2025"
  - "3D视觉"
  - "跨模态对齐"
