...
1. **跟踪对应关系（Tracking Correspondences）**:
    ...
    - 做什么：使用现有的视频目标跟踪模型（如Tracking Anything, SAMv2）从高帧率输入图像序列中提取类别无关的实例分割mask和跨帧ID
...
2. **稀疏化帧与选择显著实例（Sparsify Frames & Select Correspondences）**:
    ...
    - 做什么：从高帧率跟踪结果中均匀采样少量帧 $m \ll n$，然后选择Top-K最显著的物体实例
...
3. **可视化对应关系（Visualizing Correspondences）**:
    ...
    - 做什么：在每帧图像上为被选中的实例叠加固定大小的编号标记（圆点+数字），标记位置为实例mask的质心 $(\\bar{x}_{ij}, \\bar{y}_{ij})$
