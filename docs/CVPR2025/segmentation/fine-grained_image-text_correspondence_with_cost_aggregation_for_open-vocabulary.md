...
**本文要解决什么？** (1) 如何增强部件级图文对应而不被物体级信息所主导？(2) 如何在有限的部件标注下捕获部件-物体的组合关系？(3) 如何引入空间结构先验来区分外观相似但位置不同的部件？

**切入角度**：作者从代价聚合 (Cost Aggregation) 的视角出发，将物体和部件的图文对应分别建模为独立的代价体积，然后通过解耦的聚合策略分别优化。这个角度有希望是因为代价聚合在密集匹配任务中已被证明能有效减少匹配误差并增强泛化能力。

**核心idea一句话**：通过解耦物体级和部件级的代价体积并独立聚合，结合组合损失的部件-物体约束和 DINO 的结构引导，大幅提升开放词汇部件分割的图文对齐精度。
