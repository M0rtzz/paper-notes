...
**本文要解决什么？** (1) 定义并解决 OMG-DA（Online Model-aGnostic Domain Adaptation）这一新问题设置；(2) 在不访问任何模型信息的前提下，通过修改输入数据的分布来适应目标域；(3) 在 DR 分级任务上验证方法的有效性。
...
**核心 idea 一句话**：用 VAE 学习生成个性化非对抗扰动，以显著性图为伪标签，在模型完全不可见的条件下从数据端实现领域自适应。
