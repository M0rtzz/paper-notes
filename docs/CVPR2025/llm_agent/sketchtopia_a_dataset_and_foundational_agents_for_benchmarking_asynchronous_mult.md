...
**本文要解决什么？** (1) 构建大规模异步多模态交互数据集；(2) 设计能在异步场景中协作的多模态 agent；(3) 定义合适的评估指标衡量异步通信质量。

**切入角度**：以 Pictionary（你画我猜）游戏为载体。这个场景天然具备异步性（画者和猜者可以同时行动）、多模态性（草图 + 文本猜测 + 图标反馈）、目标驱动性（必须让对方猜中目标词）。

**核心idea一句话**：用 Pictionary 游戏作为异步多模态通信研究的 benchmark，设计 ActionDecider 实现真正的异步 agent 交互。
