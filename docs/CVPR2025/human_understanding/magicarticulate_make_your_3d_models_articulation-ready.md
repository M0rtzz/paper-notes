...
- **做什么**: 从 Objaverse-XL 中精选 33K+ 3D 模型，附带高质量骨架和蒙皮权重标注
...
**2. 自回归骨架生成（序列建模）**
- **做什么**: 将骨架表示为骨骼序列（每根骨骼 = 两端关节共 6 坐标），用 OPT-350M decoder-only Transformer 自回归生成
- **核心思路**: 
...
**3. 函数扩散蒙皮权重预测**
- **做什么**: 将蒙皮权重视为网格表面上 $\mathbb{R}^3 \to \mathbb{R}^n$ 的连续函数，用 DDPM 函数扩散学习去噪
- **核心思路**: 
