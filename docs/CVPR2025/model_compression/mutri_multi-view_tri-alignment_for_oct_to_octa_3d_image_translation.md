...
**本文要解决什么？** (1) 在离散有限空间（而非连续无限空间）中学习OCT→OCTA映射以降低不确定性；(2) 解决VQVAE在跨域翻译中的码本利用率低问题；(3) 不依赖额外的血管分割标注。

**切入角度**：利用预训练的OCT和OCTA重建模型提供高质量的领域特征作为多视图指导信号，通过对比学习最大化翻译模型与预训练模型之间的互信息，驱动码本探索更多码字。

**核心idea一句话**：预训练OCT和OCTA重建VQVAE提供三视图先验（3D OCT + 3D OCTA + 2D OCTA投影图），用对比语义对齐和血管结构对齐分别从语义和结构层面指导翻译VQVAE的码本学习。
