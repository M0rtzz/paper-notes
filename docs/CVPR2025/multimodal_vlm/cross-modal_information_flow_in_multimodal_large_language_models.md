...
**本文要解决什么？** 三个具体问题：(1) 视觉信息如何从图像 token 传播到问题 token？(2) 这种传播是粗粒度全局的还是细粒度局部的？(3) 融合后的多模态信息如何最终到达最后位置生成答案？

**切入角度**：借鉴 NLP 领域 mechanistic interpretability 中的 attention knockout 方法——通过选择性地阻断特定层中特定 token 对之间的注意力边，观察答案预测概率的变化来反向推断信息流路径。

**核心idea一句话**：用 attention knockout 逐层阻断跨模态注意力边，揭示 MLLM 中视觉-语言信息融合的两阶段机制和三步预测流程。
