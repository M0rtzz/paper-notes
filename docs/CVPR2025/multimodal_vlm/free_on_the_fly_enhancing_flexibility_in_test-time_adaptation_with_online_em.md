...
**本文要解决什么？** 如何设计一种 TTA 方法同时满足：(1) 显式建模目标域分布以利用样本间关系；(2) 不需要访问/存储训练数据或历史测试数据；(3) 无需任何梯度反传，保持高效稳定？
...
**核心idea一句话**：用 CLIP 文本嵌入初始化高斯混合模型参数，通过在线 EM 算法逐样本更新分布参数，结合 CLIP 零样本置信度加权，实现无需训练的在线域适应。
