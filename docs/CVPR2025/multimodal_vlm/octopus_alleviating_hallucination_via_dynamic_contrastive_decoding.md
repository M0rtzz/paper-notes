...
**本文要解决什么？** 设计一个能自适应地为每个样本和每个生成步骤选择最合适的 CD 策略的框架。

**切入角度**：将多种现有 CD 方法视为"诊断工具"——不是要替代它们，而是要学会在正确的时刻使用正确的工具。这类似章鱼用不同触手完成不同任务。

**核心idea一句话**：用可学习的决策模块动态选择每个生成步骤最适合的对比解码策略，实现个性化的幻觉消除。
...
**核心idea一句话**：像章鱼（Octopus）一样，用"眼睛"（decision token）识别幻觉类型，用多条"触手"（多种 CD 策略）分别应对不同的幻觉挑战。
