...
**本文要解决什么？** 1）如何让 LLM 理解和生成参数化 CAD 操作序列？2）如何构建连接自然语言与 CAD 数据的桥梁？3）如何支持从文本生成到编辑的多种 CAD 下游任务？
...
**核心idea一句话**：将 CAD 参数序列转化为附带层次化语义描述的 Python 风格代码（SPCC），然后通过自适应预训练和指令微调让 LLM 掌握 CAD 生成和编辑能力。
