...
**本文要解决什么？** (1) 简化关键点分组——把车道提取建模为图上的路径搜索问题；(2) 提高关键点检测的召回率——每个目标点允许多个提议；(3) 减少冗余提议的计算开销——PointNMS筛选。

**切入角度**：将车道检测视为有向图问题——关键点是节点，相邻关键点间的连接关系是边。分组变成了从起始节点到终止节点的最短路径搜索（Dijkstra算法），这比聚类或方向预测更简洁。

**核心idea一句话**：将车道检测建模为"检测关键点+预测有向连接+图搜索提取车道"三步流程，用多提议+PointNMS保证高召回低冗余。
