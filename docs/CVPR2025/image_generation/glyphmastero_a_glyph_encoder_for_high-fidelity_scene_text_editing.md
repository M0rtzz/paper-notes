...
4. **本文要解决什么？** 如何设计一个字形编码器，生成比现有OCR特征更细粒度、更能反映文字结构层次的引导信号，以提升扩散模型生成场景文字的准确性？
5. **切入角度**：将单个字符的glyph图像和完整文本行的glyph图像分别送入OCR模型得到局部和全局两路特征，用跨层次注意力交互让局部特征"询问"全局上下文，从而同时编码字符细节和行级结构。
6. **核心idea一句话**：设计可训练的双流字形编码器，通过glyph attention模块显式建模字符级与文本行级特征的跨层次交互，为扩散模型提供笔画级精确的条件引导。
