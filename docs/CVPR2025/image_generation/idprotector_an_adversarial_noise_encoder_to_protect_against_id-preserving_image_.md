...
**本文要解决什么？** 设计一个满足 EURI 标准（Efficiency、Universality、Robustness、Imperceptibility）的前馈式人脸隐私保护方案。

**切入角度**：将防御问题从"逐图优化"转化为"学习一个通用噪声编码器"，通过训练时联合攻击多种编码器的关键特征层来实现通用保护。

**核心idea一句话**：训练一个 ViT 编码器，一次前向传播即可预测对抗扰动，通过联合攻击 ArcFace 和 CLIP 编码器的关键嵌入层实现对多种身份保持方法的通用保护。
