...
4. **本文要解决什么**：(1) 让模型学到精准的局部语义匹配能力，即使参考图和线稿差异很大也能正确对应; (2) 提供交互式精细控制，用户可通过点对应来指导困难区域的上色
5. **切入角度**：利用动漫视频帧对作为训练数据（天然提供同一角色不同姿态的对应），通过渐进式 Patch Shuffling 打破参考图的整体结构，强迫模型学习局部匹配而非全局copy。
6. **核心 idea 一句话**：将参考图打散为 patches 后随机排列作为训练输入，迫使模型学会 patch 级局部匹配能力，辅以点控制实现精细上色引导。
