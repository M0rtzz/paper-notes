...
description: >-
  [CVPR 2025][concept learning][diffusion] 提出 ICE 框架，仅利用单个 T2I 扩散模型实现从单张图像自动提取内在概念（颜色、材质等），通过两阶段（自动定位+结构化学习）实现无监督的层次化视觉概念分解。
tags:
  - CVPR 2025
  - concept learning
  - diffusion model
...
- **做什么**: 迭代地从图像中提取概念和 mask，直到图像中无剩余物体。
- **核心流程**:
...
- **做什么**: 为每个物体级概念 $c_i$ 学习两个 token: concept-specific $c_i^{conspec}$（表示通用语义类别）和 instance-specific $c_i^{inspec}$（表示实例特有属性）。
...
- **做什么**: 将 instance-specific token 进一步分解为内在属性 token $c_j^{intrinsic}$（如颜色、材质）。
