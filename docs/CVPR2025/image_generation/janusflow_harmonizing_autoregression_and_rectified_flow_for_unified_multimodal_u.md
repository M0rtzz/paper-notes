...
description: >-
  [CVPR 2025][unified model] 提出 JanusFlow，将自回归 LLM 与 rectified flow 结合在单一模型中，通过解耦编码器和表征对齐正则化，实现多模态理解与图像生成的统一框架。
tags:
  - CVPR 2025
  - unified model
  - rectified flow
...
**本文要解决什么**: 如何以最简架构将 rectified flow（连续生成范式）与自回归 LLM 结合，在单一模型中同时实现高质量的多模态理解和图像生成。
...
**1. 解耦理解/生成编码器**
- **做什么**: 理解用预训练 SigLIP-Large-Patch/16（~300M）提取语义特征；生成用从头训练的 ConvNeXt blocks（~70M）做编码器 $g_{enc}$ 和解码器 $g_{dec}$。
...
**2. 表征对齐正则化 (REPA)**
- **做什么**: 在生成训练中，将 LLM 第 6 层的中间特征投影后与 SigLIP 编码器对真实图像的特征做余弦相似度对齐。
...
- **做什么**: 在 SDXL-VAE latent space 中训练 rectified flow，LLM 的输出经 $g_{dec}$ 变换为速度向量，用 Euler 求解器迭代更新。
