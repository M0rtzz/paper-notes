...
description: >-
  [CVPR 2025][视频生成][视频扩散模型] VideoGuide 提出了一种无需额外训练的视频扩散模型增强框架，通过在反向扩散采样的早期阶段利用任意预训练视频扩散模型（或自身）作为教师，将教师模型的去噪样本与采样模型进行插值融合，显著提升视频的时序一致性而不损害图像质量。 1. 领域现状：文本到视频（T2V）扩散模型…
tags:
  - "CVPR 2025"
  - "视频生成"
  - "视频扩散模型"
