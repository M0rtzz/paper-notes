...
description: >-
  [AAAI 2026][自动驾驶][时间预算推理] 提出TimeBill框架，通过细粒度响应长度预测器（RLP）和工作负载引导的执行时间估计器（ETE），在给定时间预算下自适应调整KV Cache驱逐比例，在保证推理完成率的同时最大化LLM响应质量。 问题背景 大语言模型（LLM）正越来越多地部署在时间关键系统中…
tags:
  - "AAAI 2026"
  - "自动驾驶"
  - "时间预算推理"
