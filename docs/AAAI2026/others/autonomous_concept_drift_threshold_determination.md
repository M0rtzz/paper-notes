...
description: >-
  [AAAI 2026][概念漂移] 证明了固定阈值不可能在所有场景下最优、动态阈值严格优于静态阈值，并提出DTD算法：在漂移检测信号触发后启动三模型比较阶段，根据候选模型表现自适应调整检测阈值。 概念漂移(Concept Drift)指数据流的底层分布随时间变化，会严重降低模型性能。漂移检测器通过监控统计量是否超过阈值来判断…
tags:
  - "AAAI 2026"
  - "概念漂移"
  - "动态阈值"
