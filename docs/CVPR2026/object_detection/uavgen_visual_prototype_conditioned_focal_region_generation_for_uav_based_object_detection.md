...
UAVGen 要解决 UAV 检测的数据稀缺，以及通用 layout-to-image 增强在小目标场景失效的问题。它把两个模块串成一条数据合成流水线：先用视觉原型条件扩散模型（VPC-DM）生成高保真布局图，再用焦点区域增强管线（FRE-DP）把生成与训练都聚焦到密集小目标区域，最后由标签精炼模块校正合成图与标注的错位，产出可直接喂给检测器的增广数据。
