...
4. **本文要解决什么？**（1）如何为每个样本自适应选择最优模型子集？（2）如何在动态选择的异构模型分数之间进行有效融合？

5. **切入角度**：将模型选择建模为 MLLM agent 的工具调用决策问题。agent 可以观察输入样本的特征，reasoning 后决定调用哪些模型，通过强化学习从结果反馈中学习最优策略。

6. **核心idea一句话**：用 MLLM agent 做样本级动态模型选择，把"用什么模型"的决策从人工规则变成可学习的 reasoning 过程，配合锚定式 top-k 分数融合实现鲁棒的选择性集成。
