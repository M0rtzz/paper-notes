...
4. **本文要解决什么**：(a) 设计统一的 4D 几何控制表示；(b) 在共享世界坐标系下实现相机与多物体运动的解耦控制；(c) 构建大规模训练数据。

5. **切入角度**：用 3D 高斯分布描述物体的概率性 3D 占据——均值定义运动路径，协方差捕捉空间范围和朝向，天然支持软性、灵活、类别无关的物体建模。

6. **核心 idea 一句话**：用静态背景点云 + 逐物体 3D 高斯轨迹在共享世界坐标系下构成统一 4D 几何状态，渲染为多通道控制图后通过 GeoAdapter 驱动冻结视频扩散模型生成。
