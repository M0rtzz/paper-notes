...
**本文要解决什么？** (1) 分解曝光和环境光照对外观的不同影响；(2) 无HDR GT情况下的显式HDR监督；(3) 极端曝光区域高斯的梯度饥饿和不足densification。

**切入角度**：从物理渲染方程出发——将高斯颜色建模为固有反射率 $H_r$ 和环境光照 $L_a$ 的函数，曝光 $t$ 和光照 $L_a$ 互补地调制动态范围。

**核心idea一句话**：将3DGS的颜色分解为反射率和光照，用曝光调制图像(IE)和光照调制高斯(GI)两个分支互补捕获HDR细节，跨分支一致性损失和光照引导梯度缩放解决HDR监督和梯度饥饿问题。
