...
flowchart TD
    ...
    C --> D["初始 object queries Q₀"]
    subgraph DEC["L 层 Transformer 解码器"]
        direction TB
        D --> E["自注意力（含 See-Query）"]
        E --> F["查询驱动特征聚合（QD）<br/>See-Query 加权选层 + 交叉注意力聚合多级特征"]
        F -->|更新 queries，逐层精炼| E
    end
    B -->|各层几何特征 F₁..F_L| F
    DEC --> G["检测头<br/>输出类别 ĉ 与 3D 框 b̂ ∈ K×7"]
```
