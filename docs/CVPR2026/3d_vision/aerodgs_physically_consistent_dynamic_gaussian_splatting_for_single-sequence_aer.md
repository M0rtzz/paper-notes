...
AeroDGS 要解决的是单目无人机航拍视频的动态 4D 重建——这是个 inherently ill-posed 的问题：UAV 沿固定航线飞，每块区域只被看到一两次，动态目标（车、人）像素占比小却运动幅度大，单目深度本就病态，叠在一起就是严重的深度歧义和不稳定运动估计。框架的思路是“以物理补几何”：先用单目几何提升（MGL）从只有单次观测的序列里抠出可靠的静态背景和动态目标初始几何，再用物理引导优化（PGO）把“物体该站地上、该直立、该平滑运动”这些常识写成可微损失去约束动态目标，消解单目歧义。两个模块联合优化，静态背景与动态实体协同精炼。

### 关键设计

**1. 单目几何提升（MGL）：从只看一次的航拍序列里抠出可靠的静态与动态几何**

航拍每块区域只被观测一两次、缺多视图冗余，几何初始化最先卡住。MGL 分三路应对：静态背景靠 SfM / 单目深度网络从 UAV 飞行带来的相机平移视差里恢复稀疏/稠密点云，初始化静态高斯；动态目标靠运动分割或检测从背景里分离——动态区域在 SfM 中会产生不一致的重投影，正好用来识别它们；每个动态目标再用单目深度和检测框估初始3D位置与形状，用3D高斯参数化，除了标准属性（位置 $\mu$、协方差 $\Sigma$、颜色 $c$、不透明度 $\alpha$）外还挂上时间相关的运动参数。这一步给后面的物理优化提供了一个虽粗但可用的起点。

**2. 地面支撑先验：禁止动态物体悬浮或穿透地面**

单目深度歧义最直接的恶果是动态物体在垂直方向乱漂——浮在空中或扎进地里。地面支撑先验先从静态背景重建或 DEM 估出地面平面，再约束动态高斯底部贴着地面，用一个可微的接触损失在物体高度偏离地面支撑位置时惩罚：

$$\mathcal{L}_{\text{ground}} = \sum_{i} \max(0, z_{\text{ground}} - z_i + \epsilon)^2$$

其中 $z_i$ 是第 $i$ 个动态高斯的垂直坐标，$z_{\text{ground}}$ 是对应地面高度。消融里加上它直接消除了地面穿透。

**3. 直立稳定性先验：让车和人在运动中保持直立不乱翻**

光约束高度还不够，物体朝向也会因歧义乱倾乱转。直立先验约束动态目标主轴与重力方向对齐，对朝向参数施加正则：

$$\mathcal{L}_{\text{upright}} = \sum_{i} (1 - |\mathbf{n}_i \cdot \mathbf{g}|)$$

其中 $\mathbf{n}_i$ 是动态目标法向量，$\mathbf{g}$ 是重力方向单位向量。点积越接近 1 惩罚越小，于是姿态被拉回直立。

**4. 轨迹平滑性先验：用惯性约束抑制运动的瞬间跳跃和抖动**

逐帧独立估计的运动会出现瞬移和高频抖动，不符合惯性。平滑先验对动态高斯的时序位移施加加速度正则，惩罚相邻时刻的速度突变：

$$\mathcal{L}_{\text{smooth}} = \sum_{t} \| (\mu_{t+1} - \mu_t) - (\mu_t - \mu_{t-1}) \|^2$$

它等价于惩罚二阶时间导数（加速度），允许匀速运动但压住高频抖动——既不把运动锁死，又把不合理的跳变滤掉。
