...
**本文要解决什么？** 定义了一个全新的任务 Image2AVScene:从单张图片同时生成可交互的3D视觉场景和与之语义/几何对齐的空间声场，并提出了首个完整框架。

**切入角度**：采用全景表示（equirectangular panorama）统一视觉和音频的坐标系，并利用VLM进行语义理解来桥接视觉与声音。

**核心idea一句话**：通过全景扩绘→3DGS重建→VLM驱动的360°语义定位→Ambisonics编码的无训练流水线，实现从单图生成可自由漫游的3D音频-视觉场景。
