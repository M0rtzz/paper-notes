...
通用 IQA 只看美学或低级失真，刻画不了电商海报的功能性。论文与资深电商美术总监合作，把质量拆成四个正交维度——Object（产品完整性/清晰度）、Background（背景兼容性/视觉吸引力）、Text（排版可读性/正确性）、Layout（整体构图/视觉层次）。维度间平均 Pearson 相关只有 $\rho \approx 0.24$，说明单一整体分确实盖不住电商海报的质量结构；进一步的"最薄弱环节"分析显示 Text 是 44.8% 低质图像的瓶颈，且与整体质量相关性最高（$\rho=0.67$）——这也解释了为什么强模型一旦漏判笔画级文字错误就满盘皆输。
...
E-comIQ-M 以 Qwen-2.5-VL-7B 为底座分两步训练：Stage 1 在 15K 训练集上 SFT，学领域知识和输出格式；Stage 2 用 GRPO 在按 SFT 模型 MSE 排序取出的 worst 3K 困难样本子集 $\mathcal{D}_{hard}$ 上做强化优化。奖励函数为 $R(x,y) = R_{score}(x,y) + \lambda_{fmt} R_{fmt}(y)$，其中 $R_{score}$ 对齐专家分数、$R_{fmt}$ 确保输出是合法 JSON。先 SFT 打底再 GRPO 在硬样本上精修，比单跑任一阶段都更贴近人类判断。整套标注还靠质量保证流程托底：六名领域专家先在 1000 张校准集上交叉标注、达成 Krippendorff's $\alpha = 0.858$ 后再分工，并保持 10% 随机抽样防止标准漂移。
