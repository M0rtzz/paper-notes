...
description: >-
  [CVPR 2026][优化/理论][解析联邦学习] SAFLe 用「特征分桶 + 打散分组 + 稀疏嵌入求和」搭出一个确定性的非线性分类头，并证明它在数学上等价于一个高维稀疏线性回归，从而能直接套用解析联邦学习（AFL）的单轮闭式聚合律——既拿到非线性的表达力，又保留 AFL「一轮通信 + 对数据异质性完全不变」的两大优势，在三个视觉联邦基准上同时超过线性 AFL 和多轮 DeepAFL。
tags:
  - "CVPR 2026"
  - "优化/理论"
  - "解析联邦学习"
