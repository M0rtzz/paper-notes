...
description: >-
  [CVPR 2026][视频理解][脉冲相机] 针对脉冲相机超分（SCSR）只能处理固定短序列、且脉冲信号有波动的两大痛点，Spk2VidNet 用「逐层放大时间感受野的两层递归传播 + 多帧一致性对齐 + 内容感知调制融合 + 分段训练状态传递」从任意长脉冲流重建高分辨率图像序列，在合成与真实数据上以更快速度刷新 SOTA（REDS-LSSR ×4 PSNR 29.92dB、推理仅 43ms）。
tags:
  - "CVPR 2026"
  - "视频理解"
  - "脉冲相机"
