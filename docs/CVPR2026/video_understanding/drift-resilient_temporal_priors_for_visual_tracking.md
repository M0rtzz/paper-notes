...
DTPTrack 是一个即插即用的时序模块，插在主 Transformer block 之前，专治跟踪器的「模型漂移」——一帧错了就被烘焙进时序模型、级联拖垮后续。它每次处理五帧：初始模板 $z_0$（来自 GT）、三个历史参考帧 $z_1, z_2, z_3$（前三个时间步的搜索区域）和当前搜索区域 $x_0$。主骨干基于扩展的 LoRATv2，用帧内因果注意力（FWCA，帧内全注意力 + 跨帧因果注意力）兼顾空间推理与时序依赖，再为每条输入流配一个流特定 LoRA 适配器（SSLA）共享冻结 ViT。在此之上，模块先用 TRC 给历史帧打可靠性分、过滤噪声，再用 TGS 把校准后的历史合成成动态先验 token 注入。
