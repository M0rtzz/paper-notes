...
LongVideo-R1 要解决的是长视频理解的算力痛点：暴力管线把整段视频切片逐段处理，开销随时长线性爆炸。它换成「像人一样先看概要、再定向钻进感兴趣片段」的主动导航——把长视频组织成深度 $D=3$ 的多层树，每个非叶节点有 $K = \text{round}(\sqrt[D]{T/16s})$ 个子节点，叶节点对应约 16 秒短片段。一个 LRM（Qwen3-8B 微调）作为大脑，配两个多模态工具做 Chain-of-Thought-with-Tool（CoTwT）推理，平均只需约 10.5 轮调用就能答完一题。
