...
**5. 弱监督就绪性学习：不要证据时间戳标注也能学会判断时机**

现有流式基准没有证据时间窗口标注，没法直接监督"何时答"。训练时它利用 $z_\ell$ 与 $\mathcal{M}_{V2}$ 的时序相似度构建伪正/负区域，用对比损失 $\mathcal{L}_{ctr}$ 学就绪性，绕开真实时间戳；选 $\mathcal{M}_{V2}$ 这一层是因为它在细节和紧凑性之间最平衡。

**6. 梯度隔离设计：让两个目标互不打架**

"答什么"和"何时答"如果共享梯度容易相互干扰。$\mathcal{L}_{rdy}$ 只更新 Readiness Head 和 `<RDY>` token，不把梯度回传到推理模块，让两个目标各自独立优化。
