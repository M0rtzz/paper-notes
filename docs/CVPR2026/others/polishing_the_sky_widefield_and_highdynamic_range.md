...
### 整体框架
POLISH++ 要回答一个很实际的问题：怎么让深度学习射电成像方法真正搬得上 DSA 这种下一代巡天望远镜。它本身是个基于 WDSR 的端到端 CNN，输入低分辨率 dirty image、输出 2 倍超分辨的 clean sky，但真正让它可部署的是外围三步：先对输入做 arcsinh 变换把跨数量级的动态范围压到同一量级，再让 CNN 在变换空间里前向重建，最后用逆 arcsinh 变换还原回真实强度；训练和推理都在 patch 级别进行，推理后把各块拼回完整视场。模型沿 POLISH → POLISH+（加分块训练）→ POLISH++（再加 arcsinh）逐步演进。
