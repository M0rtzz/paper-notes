...
description: >-
  [CVPR 2026][电磁逆散射] 本文把电磁逆散射问题（EISP）从"逐样本物理优化"改写成"端到端数据驱动回归"——用一个 MLP 直接把接收到的散射场和空间坐标映射成该点的相对介电常数，靠训练集学到的数据分布先验补偿稀疏测量的信息缺失，首次实现了仅用**单个发射机**的高质量重建，且推理比此前 SOTA 快 7 万倍。
tags:
  - "CVPR 2026"
  - "电磁逆散射"
  - "相对介电常数重建"
