...
MyoVision 想用一部智能手机就完成原本要靠高光谱/近红外专业设备的鸡胸肉肌病检测。它走的是一条"成像→手工特征→集成分类"的三段流水线：先用手机白光透射拍 14-bit RAW 图，再从图里抽 16 维结构纹理描述符（梯度统计、频域纹理响应、密实组织特征），最后交给 NEATBoost-Attention 集成模型做木质胸/意面肉/正常三分类。
