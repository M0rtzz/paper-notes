...
ViT3 想把 Test-Time Training（TTT）真正用好在视觉上。TTT 的核心是把注意力重新表述成一个在线学习问题：每一层不再做 softmax 注意力，而是把当前序列的 Key-Value 对当成一个「迷你数据集」，在测试时现场训练一个紧凑的内部模型 $F_W$，再用训练好的 $F_{W^*}$ 去处理 Query 得到输出。宏观架构和 Transformer 一样（逐层把 Attention 换成 TTT 层），复杂度从 $O(N^2)$ 降到线性。难点不在框架，而在 TTT 巨大的设计空间（内循环用什么损失、学习率、批量、epoch，内部模型用什么架构、多大）此前缺乏系统理解，导致视觉 TTT 性能被锁死。这篇论文的贡献就是系统扫这个设计空间，提炼出六条洞察并据此搭出 ViT3。
