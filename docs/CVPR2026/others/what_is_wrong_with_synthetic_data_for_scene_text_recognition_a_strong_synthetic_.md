...
这篇论文的出发点是诊断「现有渲染合成数据到底差在哪」，结论是语料、字体、布局三方面多样性都不够，于是给出 UnionST 渲染引擎逐一补齐，再叠一个自演化学习框架 SEL 把少量真实数据的价值榨干。UnionST 的渲染流程是：从增强语料库采样文本并选择兼容字体 → 逐字符独立渲染并计算位置/方向/大小参数 → 施加弹性变形、透视变换、边框等效果 → 选背景并按颜色对应表确定文字颜色，叠加随机文字实例、阴影、浮雕，输出合成图像与标签。
