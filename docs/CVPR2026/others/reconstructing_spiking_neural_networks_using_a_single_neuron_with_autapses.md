...
description: >-
  [CVPR 2026][脉冲神经网络] 受小脑浦肯野细胞自突触自反馈启发，本文给 LIF 神经元加上一组「时延自突触」（TDA-LIF），让**单个**脉冲神经元在时间维度展开后，通过裁剪/共享自突触就能等价重构出储备池（RC）、多层感知机（MLP）和类卷积三种 SNN 结构；在 RC/MLP 上达到与同规模标准 SNN 相当的精度，同时把每层神经元数压到 1、状态显存从 8 KB 降到 4 Byte、单神经元信息密度提升几十倍，代价是极端单神经元设置下的时间延迟。
tags:
  - "CVPR 2026"
  - "脉冲神经网络"
  - "自突触"
