...
DiffBMP 要解决的是 2D 可微渲染长期只能处理矢量图形、碰不了位图的空白。整条 pipeline 是：输入一组位图图元和目标图像 → 通过可微前向渲染（坐标变换 + 双线性插值采样 + Porter-Duff alpha 合成）得到渲染结果 → 计算损失 → 通过自定义 CUDA 反向传播内核高效求梯度 → 更新每个图元的 $(x_i, y_i, s_i, \theta_i, \nu_i, \mathbf{c}_i)$ → 收敛后导出分层 PSD 文件。

### 关键设计

**1. 可微前向渲染：把位图组合渲染做成全程可微 + tile 并行**

位图是离散的高维像素数组，直接组合渲染既不可微、又有巨大的内存和计算开销。对画布像素 $(x,y)$，先经旋转矩阵和平移/缩放变换映射到图元的归一化坐标 $(u,v) \in [-1,1]^2$，转为离散坐标 $(U,V)$ 后用双线性插值采样，使空间变换完全可微；每个图元的 alpha 值 $= \alpha_{\max} \cdot \sigma(\nu_i) \cdot M_i(x,y)$，按标准 Porter-Duff over 合成累积透射率和最终颜色。计算上把画布分为 $T \times T$ 的 tile（默认 $T=32$），每个 tile 交给一个 CUDA thread block 处理，实现像素级完全并行。

**2. 软光栅化（Soft Rasterization）：把稀疏梯度抹宽**

双线性插值的梯度仅在物体边界非零、极其稀疏，优化容易卡死。对图元施加高斯模糊以扩展梯度的空间作用范围，让远离边界的像素也能回传梯度，实验证明可稳定提升 PSNR（Tab. 3）。

**3. 结构感知初始化（Structure-aware Init）：按目标复杂度放图元**

随机初始化会让图元放错位置、收敛缓慢。利用目标图像 $7 \times 7$ 窗口的局部方差指导图元放置——高方差区域密集放置小图元、低方差区域稀疏放置大图元，颜色初始化为目标像素值加噪声，让初始布局就贴合目标结构。

**4. 噪声画布（Noisy Canvas）：逼图元去覆盖同色区域**

当前景与背景同色时图元会"偷懒"留下空洞。把背景设为均匀随机噪声 $\mathbf{b}(x,y) \sim \mathcal{U}[0,1]^3$，迫使图元覆盖那些与背景同色的区域，避免空洞。

**5. 颜色约束：可选保留图元原始颜色**

品牌 logo 拼图等应用需要保留图元本身的颜色。用可选参数 $\mu_{\text{blend}}$ 控制是否保留图元原色，$\mu_{\text{blend}}=1$ 时完全保留原色。

**6. 高效反向传播：FP16 混合精度 + 专用导出内核**

数千图元的高分辨率梯度对带宽和显存压力极大。梯度通过链式法则从渲染输出精确传播到各图元参数（位置、缩放、旋转、颜色、透明度），均可精确计算无需近似；实现上采用 FP16 半精度 + `__half2` 打包 + `atomicAdd` 大幅降低带宽和显存占用，并用专用导出 CUDA 内核支持高分辨率 PSD 导出（优化时低分辨率，导出时 2×/4× 高分辨率）。
