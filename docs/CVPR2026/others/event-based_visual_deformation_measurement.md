...
description: >-
  [CVPR 2026][事件相机] 本文提出一套事件—帧融合的视觉形变测量（VDM）系统，用事件相机提供时间稠密的运动线索、用普通帧提供空间稠密的精确约束，并通过仿射不变单纯形（AIS）框架把高维形变场切成低参数三角子区域、再配合邻域贪心优化抑制长程误差累积，在 100+ 像素大形变下把追踪存活率做到 SOTA 的 1.6 倍，而存储/算力只占高速相机方案的 18.9%。
tags:
  - "CVPR 2026"
  - "事件相机"
  - "视觉形变测量"
