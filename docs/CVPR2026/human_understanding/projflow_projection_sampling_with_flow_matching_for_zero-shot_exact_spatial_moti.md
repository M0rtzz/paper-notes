...
description: >-
  [CVPR 2026][人体理解][流匹配] 把一大类人体运动控制任务（轨迹跟随、2D→3D 抬升、运动补全、循环动作等）统一成线性逆问题，提出 ProjFlow——一个无需训练的流匹配采样器，在每一步去噪时用闭式投影把"干净运动估计"拉到约束集上，并用一个编码骨架拓扑的"运动学感知度量"让校正沿骨骼协调扩散，从而在零样本、无内层优化的条件下**精确**满足硬约束、同时保持运动自然度。
tags:
  - "CVPR 2026"
  - "人体理解"
  - "流匹配"
