...
description: >-
  [CVPR 2026][人体理解][长时运动嵌入] 与其用视频生成模型逐像素同时建模"外观+运动"，本文直接学一个**只编码运动、时间压缩 64× 的长时运动隐空间**：先用轨迹 VAE 把稀疏跟踪轨迹压成稠密可查询的运动网格，再在这个空间里训一个条件流匹配模型按文本/戳点（poke）生成长时目标导向运动，比 SOTA 视频模型快一万倍以上、质量还更好。
tags:
  - "CVPR 2026"
  - "人体理解"
  - "长时运动嵌入"
