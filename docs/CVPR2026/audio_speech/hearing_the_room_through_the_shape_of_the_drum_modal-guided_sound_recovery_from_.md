...
description: >-
  [CVPR 2026][音频/语音][散斑测振] 针对响应差、强共振的"硬"物体（鼓面、笔记本、相框等），本文用散斑测振一次性采集物体表面 10×10 个点的双轴振动，推导一个把"场景声音 → 多点振动"联系起来的物理前向模型（以物体的振动模态为桥梁），再通过优化反演这个模型，把几十路含噪振动融合成一路去噪、去共振音色的声音，质量显著优于单点散斑测振和经典信号处理融合（平均、delay-and-sum）。
tags:
  - "CVPR 2026"
  - "音频/语音"
  - "散斑测振"
