...
诊断出基础模型普遍不鲁棒后，本文提出鲁棒性感知微调来补救：优化目标是效用损失和鲁棒性损失的加权和——效用项保持原有下游表示质量，鲁棒性项最小化扰动图像之间的嵌入变化。实验证实这样能在多数扰动类型上提升鲁棒性而不损害效用，说明鲁棒和好用并不必然冲突。

### 损失函数 / 训练策略

微调损失 = 效用损失（保持原有表示质量）+ $\alpha$ × 鲁棒性损失（最小化扰动嵌入变化），$\alpha$ 控制两者平衡。
