...
**3. 复合 FASS 分数：三种失败模式等权合一**

为了给出一个可比较的总分，三个指标取等权平均 $\text{FASS} = (S + R + J)/3$。等权意味着把空间位移、排序变化、显著区域漂移视为同等重要，不预设哪种失败更严重——具体应用想偏重某一轴时再自行调权。

**4. 扰动分类：跳出 ε-ball 加性噪声**

现有框架几乎只在加性噪声下测稳定性，而真实系统里更常见的是几何、光度、压缩这类结构化扰动。FASS 把扰动分成三类系统施加：几何（15° 旋转、20 像素水平平移，零填充边界）、光度（亮度缩放 ×1.5、高斯噪声 $\sigma=0.15$）、压缩（JPEG 质量因子 40）。正是这组扰动暴露出"仅靠加性噪声会系统性高估归因鲁棒性"。
