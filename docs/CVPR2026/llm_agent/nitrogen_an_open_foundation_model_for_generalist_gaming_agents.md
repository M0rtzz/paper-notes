...
description: >-
  [CVPR 2026][LLM Agent][游戏 Agent] NitroGen 把「玩家在直播画面里叠加的手柄按键 overlay」当成天然动作标签，从 4 万小时、1000+ 款游戏的公开视频里自动抽出 (画面, 动作) 对，用 flow-matching 训练一个单一的视觉-动作 Transformer，让一个模型在 2D/3D 多种游戏里直接玩，且预训练权重微调到没见过的游戏上成功率最高相对提升 52%。
tags:
  - "CVPR 2026"
  - "LLM Agent"
  - "游戏 Agent"
