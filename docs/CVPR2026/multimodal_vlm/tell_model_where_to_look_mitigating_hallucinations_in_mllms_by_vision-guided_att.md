...
### 整体框架
VGA 的出发点是一个观察：MLLM 其实能从视觉 token 里准确提取物体的语义特征并转成条件概率（visual logits），但推理时没把这份能力用起来，视觉理解被低估了。于是 VGA 分两步且免训练——先用 Visual Semantic Confidence (VSC) 从 visual logits 构建出精确的视觉定位，再用这个定位去引导模型注意力聚焦到相关区域。关键是整个过程每个 token 只需一次前向传播，且全程不依赖 attention weight，因此兼容 FlashAttention。

### 关键设计

**1. Visual Semantic Confidence (VSC)：从 visual logits 里读出物体定位**
...
**2. Visual Semantic Salience (VSS)：给 captioning 这类无目标任务定位**
...
**3. Vision-Guided Attention (VGA)：用加法结合律绕开 attention weight**
...
**4. Attention Heads Balancing：别把本来就擅长看图的头搞坏**
...
**5. Programmed Visual Grounding (PVG)：随生成动态挪动引导焦点**
