...
Reason-SVG 要解决的是 LLM 把复杂组合式提示（如“a white-and-red castle on a floating island among clouds”）直接映射到 SVG 代码时的语义鸿沟。它的思路是“先规划后绘制”：给定文本提示 $\mathcal{T}$，模型先生成一段 DwT 推理链 $C$ 充当概念脚手架，再据此生成 SVG 代码 $O$，整体是 $\Phi: \mathcal{T} \rightarrow (C, O)$。训练分两步走——SFT 先把这套显式推理能力灌进模型，GRPO 再用一个混合奖励把推理质量和生成质量一起拉高。
