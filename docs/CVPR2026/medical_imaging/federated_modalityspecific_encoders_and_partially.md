...
4. **本文要解决什么？** (a) 如何处理不同客户端拥有不同模态组合带来的模态间异质性？(b) 如何在联邦共享通用知识的同时，实现每个客户端的有效个性化？
...
6. **核心 idea 一句话**：用模态专属编码器处理模态间异质性，用基于参数更新一致性的动态部分个性化解码器平衡知识共享与个性化，再用多锚点多模态表征校准缺失模态的信息缺口。
