...
MedCLIPSeg 想同时解决医学分割的三个老问题——标注稀缺、边界模糊、跨设备域偏移——而它的抓手是把 CLIP 的跨模态先验用得更细。框架建在冻结的 CLIP 双编码器之上（默认 UniMedCLIP 骨干）：输入医学图像经视觉编码器得到 patch 级嵌入，文本描述（含器官位置、成像模态等）经文本编码器得到 token 级嵌入；在 CLIP 中间层插入可学习的**概率视觉-语言（PVL）适配器**做图文双向融合，融合后的视觉特征再送进轻量分割解码器出掩码。训练损失由分割损失和软 patch 级对比损失组成，全程只训练适配器和解码器。

### 关键设计

**1. 概率交叉模态注意力（PVL Adapter）：把 Key/Value 建成分布，一并搮定融合与不确定性**

现有 CLIP 适配多是单向的文本→图像引导，且用确定性注意力，没法表达"这个 patch 特征到底可不可信"。PVL 的做法是把每个 token 的 Key 和 Value 不再当成确定向量，而建成高斯分布：通过可学习投影分别预测均值与方差，得到 $\text{Key} \sim \mathcal{N}(\mu_K, \sigma_K^2)$、$\text{Value} \sim \mathcal{N}(\mu_V, \sigma_V^2)$，高方差代表语义不确定、低方差代表高置信。注意力权重不只看 Query-Key 相似度，还被 Key 方差的倒数（即置信度）加权、由超参 $\beta$ 控制，于是不确定的 token 会被自动降权。同时 PVL 双向作用——视觉 patch 查询文本 token（text→image）、文本 token 查询视觉 patch（image→text），再用残差门控把适配特征和原始 CLIP 特征融合。把不确定性塑进注意力阶段，模型在融合时就能抑制噪声和模糊特征，对医学图像常见的边界模糊和伪影尤其管用。

**2. 像素级不确定性估计：复用 Value 分布，免费产出一张可信度图**

确定性模型只给一个分割结果，临床上却需要知道哪些区域可信。MedCLIPSeg 直接复用 Value 的概率分布，在推理时用 Monte Carlo 采样跑多次预测，取均值作为最终掩码、算熵作为逐像素不确定性图。这张图把分割歧义直观地标出来，且实验显示不确定性热点稳定落在解剖边界和困难区域、域内域外一致，可当作部署时的自动质控信号。

**3. 软 Patch 级对比损失：把图文对齐从图像级细化到 patch 级、从硬标签换成软目标**

全局图文对比损失只在图像级对齐，粒度太粗，鼓励不出细腻的语义差异。MedCLIPSeg 改在 patch 级做对比，且用软目标而非硬正负样本：对每个视觉 patch，算它与多组不同文本描述（同义改写、不同细节程度）的相似度，softmax 成软分布当监督信号。这让模型学的是"这个 patch 更像哪种描述"的连续语义关系，而非简单的匹配/不匹配二值判断，从而在有限标注下泛化更好。
