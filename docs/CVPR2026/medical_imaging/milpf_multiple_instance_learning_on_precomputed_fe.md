...
**1. 双流嵌入数据集构建：用一次编码同时拿到「整体密度」和「稀疏病灶」两套信号**

乳腺 X 线的难点在于，致病信号既藏在整体组织密度这种宏观结构里，也藏在一两个稀疏的小病灶里——单看哪一路都会丢信息。MIL-PF 因此让冻结编码器 $\mathcal{F}$ 对每次检查跑两遍。全局流直接编码整张图，$\mathcal{G}_i = \{\mathcal{F}(I_i^{(n)})\}_n$，每个视角一个向量，负责捕捉整体组织密度的宏观背景。局部流则把图像切成不重叠的 tile 网格，先丢掉纯背景 tile，再对含乳腺组织的 tile 逐个编码，把所有视角的 tile 嵌入并到一起 $\mathcal{T}_i = \bigcup_n \bigcup_k \{\mathcal{F}(C_i^{(n)(k)})\}$。这里 tile 尺寸是个权衡量：要大到能装下预期的 ROI，又不能超过编码器支持的最大分辨率（448 或 518 像素）。两路特征都预先算好存成嵌入数据集，后续训练只动那个 40k 参数的头，这正是「5 分钟训练」的来源。

**2. Perceiver 风格局部注意力聚合器：从一堆 tile 里只「拎」出和病变相关的那几个**

局部流会产生大量 tile 嵌入，而真正含病灶的往往只有少数几个。如果用 mean pooling，少数病灶信号会被一大片正常组织 tile 稀释；用 max pooling 又只能抓到单个最显著的 tile，多病灶时会漏。MIL-PF 借鉴 Perceiver 的做法：引入一个可训练的 latent 向量 $z$ 当 query，把所有 tile 嵌入投影成 Key 和 Value，按

$$\mathcal{A}_\omega^T(\mathcal{T}_i) = \text{softmax}(zK^\top)V$$

做加权汇总成单个向量。注意力权重让模型自己学会该关注哪些 tile，从而在稀疏 ROI 场景下既不被背景淹没、也不止盯一个点。值得注意的是只需 1 个 latent query 就够了，再加并无收益；相比建模 tile 之间两两依赖的 self-attention，这种单 query 交叉注意力参数更省，正契合「轻量头」的目标。
...
其中聚合器内部只是个 2 层 MLP（embedding_dim→16→8，ReLU），所以整个头才 ~40k 参数。之所以不在编码阶段就做更复杂的早期融合，是因为两路信号语义不同（一路是整图密度、一路是病灶响应），晚期拼接既保住了模块化和可解释性，又在这个任务上没观察到早期融合带来额外收益。
