...
SegAnyPET 想解决的是「全身 PET 影像缺乏解剖对比度、又没有现成大模型可用」的困境，做法是把 SAM 的「图像编码器 + 提示编码器 + 掩码解码器」三件套整体搬到 3D，让分割直接发生在体积上而不是逐切片拼接。一张原始 PET 体积 $X \in \mathbb{R}^{H \times W \times D}$ 进来，图像编码器 $\mathcal{E}_I$ 配合 3D 绝对位置编码抽出体素级特征；用户给的点击点（稀疏提示，经 3D 位置编码）和粗掩码（密集提示，经 3D 卷积 + LayerNorm + GELU）由提示编码器 $\mathcal{E}_P$ 对齐到同一潜空间；掩码解码器 $\mathcal{D}$ 用 3D Transformer 块融合两路信息、再经 3D 转置卷积逐步上采样，吐出体积级分割掩码。整个交互可以反复进行，预测越改越准。

### 关键设计

**1. 迭代评估循环：把医生「点一下、看一眼、再点」搬进训练和推理**

PET 边界天然模糊，一次点击很难框准，所以模型设计成可多轮细化。每轮预测后自动拿 $\hat{Y}^{(t)}$ 和 GT 比对，从假阴性区采正点、假阳性区采负点，把新点击累积进提示再预测一轮：$\hat{Y}^{(t+1)} = \mathcal{D}(\mathcal{E}_I(X), \mathcal{E}_P(p^{(t)}, \hat{Y}^{(t)}))$。人类的纠错信号被直接编码成下一轮输入，分割随点击数增加单调变好，而不是一锤定音。

**2. 双变体设计：一个管器官，一个专攻病灶**
...
**4. Patch-based 推理：用滑窗扛住整幅全身体积**

全身 PET 体积太大无法整幅塞进显存，推理时裁成 $128^3$ 的 patch 逐块预测；当预测区域触及 patch 边界、说明目标可能被切断，就启用 50% 重叠的滑窗策略把跨块结构接回来。
