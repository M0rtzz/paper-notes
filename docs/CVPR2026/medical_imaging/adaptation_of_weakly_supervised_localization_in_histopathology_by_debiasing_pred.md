...
偏差矫正的第一步是定位"问题样本"。作者不对全部 dominant class 样本一刀切，而是只盯住其中模型自己都拿不准的那批。先用归一化熵 $H(x)$ 度量每个预测的不确定性，再框定 dominant class 样本集 $\mathbb{B} = \{x \in \mathbb{T}: \hat{y}(x) \in \mathcal{B}\}$，从中取熵最高的 top-$\rho$ 子集构成 forget set $\mathbb{B}_f = \text{top}_\rho(\mathbb{B}, H(x))$。之所以挑高熵样本，是因为它们本就趴在决策边界附近——模型把它们归到 dominant class 时信心最虚，最可能是被强行误判的那部分。对这些样本动手，矫正性价比最高，又不会误伤模型确信无疑的正确预测。

**2. Forget Loss：用一个反向交叉熵让模型"松口"**

挑出 forget set 后，关键是怎么让模型放弃这些偏向性预测。作者设计的遗忘损失 $\mathcal{L}_{\text{forget}} = \mathbb{E}_{x_i \in \mathbb{B}_f}[-\log(1 - p_i(\hat{y}))]$ 形式上极简，却正好和标准交叉熵反着来：最小化它等价于**最大化**模型对当前伪标签 $\hat{y}$ 的交叉熵，相当于主动推低模型在这些样本上对 dominant class 的置信，把它们从坍缩的决策面上"拽回来"。这一项不是孤立工作的，它和作用在 retain set 上的标准交叉熵 $\mathcal{L}_{\text{retain}} = \mathbb{E}_{x_i \in \mathbb{B}_r}[-\log(p_i(\hat{y}))]$ 一推一拉：可靠样本的预测被锚住不动，不确定样本的偏向被松开，两股力合在一起把类别边界重新拉到合理位置——这正是机器遗忘思路的落点，遗忘的不是某个类别或源知识，而是那条画歪了的边界。

**3. 像素级定位损失：给定位能力单独上一道锚**

只在分类层面纠偏还不够。跨域之后目标 ROI 的外观可能和源域截然不同，光把分类边界扳正，定位能力仍可能在适配过程中悄悄漂走。为此作者额外训练一个轻量像素级分类器 $h$，把每个像素判成前景（ROI）或背景。监督信号是这样攒出来的：对每个预测类 $k$，先选熵最低、最可靠的样本子集 $D_{\text{loc}}$，再从源模型的 CAM 抽出像素级伪标签 $\bm{Y}$，然后用二值交叉熵训 $h$：

$$\mathcal{L}_{\text{loc}} = -(1-\bm{Y}_p)\log(h(z_p)_0) - \bm{Y}_p\log(h(z_p)_1)$$

挑低熵样本来生成伪标签是关键——它保证用来锚定定位的监督本身够干净，让模型在偏差矫正的同时不丢掉对 ROI 的空间感知。

**4. 动态重采样：让遗忘决策可回退**

forget/retain 的划分不是一锤定音的。初期模型偏差最重，此时算出的熵和预测分布并不可靠，若把第一轮的划分固定下来，早期的错误遗忘就会不可逆地累积。作者的做法是每 $m$ 个 epoch 用当前模型重新计算预测分布和熵，重建 forget/retain sets。随着边界逐步移动，之前被判为不确定的 forget 样本可能已经变得可靠、回到 retain 一侧，而原先稳妥的 retain 样本也可能露出新的偏向、需要被遗忘——这种周期性重建实际上扮演了隐式课程学习的角色，跟着边界的演化动态调整矫正对象。
