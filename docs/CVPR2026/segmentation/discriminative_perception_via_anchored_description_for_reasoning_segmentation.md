...
### 整体框架
DPAD 针对 RL+GRPO 训练推理分割时的盲点：geometric reward（IoU/L1）只看最终分割准不准，管不了 reasoning chain 是不是真的聚焦目标，于是推理链越拉越长、越来越发散却换不来更好的分割。DPAD 在 GRPO 框架上做两处扩展：输出端让 MLLM 在 reasoning chain $T$ 和定位 $A$ 之外，额外吐一段描述目标的 **anchored description** $C$；奖励端在 geometric reward 之外加一个基于 CLIP 的 **Discriminative Perception Reward** $R_{dpad}$，用它逼描述变得有判别性，从而间接把推理链摁回到目标的独有属性上。

### 关键设计

**1. Anchored Description：把推理链的内部理解外化成可评估的文本**

推理过程本身没法直接打分，DPAD 让 MLLM 把输出扩成三件套：reasoning chain $T$（多步推理）、geometric localization $A$（bbox 或 mask 坐标）、以及 anchored description $C$（锚定在目标视觉属性上的描述性 caption）。这段 description 是连接推理链和判别奖励的桥——它把推理链里"我理解到了什么"显式写成一句话，于是可以用 CLIP 去衡量这句话到底抓没抓住目标。

**2. Discriminative Perception Reward：用 ROI 对 AOI 的相似度差逼出判别性**

核心思路是：一段好的描述应该贴着目标区域、而不像在描述整张图——因为它讲的该是目标的独有属性，不是全图的通用特征。具体用 CLIP 文本编码器取 $C$ 的特征 $V_C$，视觉编码器分别取目标区域（ROI）和全图（AOI）的特征 $V_{ROI}, V_{AOI}$，算两个余弦相似度并取其正向差：
...
若描述只说"图里有个物体"，$V_C$ 跟 ROI、AOI 都差不多，$\Delta \approx 0$、reward=0；若描述了"红色条纹的椅子"这种独有属性，$V_C$ 会更贴 ROI，$\Delta > 0$、reward=1。要拿到这个奖励，reasoning chain 就不得不聚焦目标的 unique attributes，推理链质量被间接约束。用相对差 $S_1 - S_2$ 而非绝对阈值，还省去了校准相似度绝对数值的麻烦、更鲁棒。

**3. 综合奖励与 GRPO 优化**

三种奖励合成最终信号 $R_{final} = R_{format} + R_{geo} + R_{dpad}$：$R_{format}$ 确保输出遵循"推理 + 定位 + 描述"的格式（缺它格式会乱、其他 reward 算不出来），$R_{geo}$ 基于 IoU/L1 评估几何精度，$R_{dpad}$ 评估描述的判别性。优化用 GRPO——对同一 query 采样 $G$ 个候选，按组内相对排名估计策略梯度更新 MLLM，其中 CLIP 作为 reward model 的一部分全程 frozen、不参与梯度。

### 损失函数 / 训练策略
训练走标准 RL pipeline，GRPO 采样组大小为 $G$ 做组内相对排名，CLIP 冻结充当 reward model，训练数据用 ReasonSeg 训练集。
