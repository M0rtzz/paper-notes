...
description: >-
  [CVPR2026][语义分割][遥感图像显著性检测] 针对遥感图像中目标尺度变化大的难题，提出区域比例感知的动态自适应显著性目标检测网络 RDNet，通过 Proportion Guidance 动态选择不同大小卷积核组合，结合小波频域交互与交叉注意力定位模块，在三个 ORSI-SOD 数据集上全面超越 SOTA。
tags:
  - "CVPR2026"
  - "语义分割"
  - "遥感图像显著性检测"
...
多数方法对所有样本用同一套多尺度卷积，没考虑不同图里目标占比的差异。RPL 对 $F_4^R$、$F_5^R$ 先做**通道注意力**（GAP + 两层 1×1 Conv + Sigmoid）交叉乘-加优化通道，再做**空间注意力**（通道维 Max Pool + Sigmoid）交叉乘-加优化空间，拼接后 3×3 Conv 得定位特征 $F^A$。关键的 **Proportion Guidance (PG) Block** 对 $F_5^R$ 做 GAP → 两层 FC，输出每个样本的目标区域比例 $F^G \in \mathbb{R}^{4 \times 1}$，并用 MSE Loss 监督——这个比例就是后面 DAD 动态选核的依据。

**2. 动态自适应细节感知模块（DAD）：按区域比例切换卷积核组合**

固定核对不同尺度目标会失配：大核抓大目标但给小目标引背景噪声，小核反之。DAD 用 PG 估出的比例把目标分三档动态选核：
...
具体走双支路：下支路（Detail Extractor）多核卷积后相加融合得 $F_1^D$；上支路（Detail Optimizer）先通道维 Max Pool，再用相同核组合相加、1×1 Conv + Sigmoid 得权重 $W$；最终 $F^P = F_1^D \otimes W \oplus F_1^D$。这把"先分类目标大小、再决定卷积策略"显式做进了网络。

**3. 频率匹配上下文增强模块（FCE）：把层间交互搬到小波频域**

现有方法在全分辨率特征上直接做 self-attention，计算贵且高低频混在一起稀释目标信息。FCE 先做**小波交互**：对 $F_2^R$、$F_3^R$ 做离散小波变换（DWT）得 LL/LH/HL/HH 四个频率分量，对应频率分量之间做矩阵乘法交互（reshape → 转置 → 矩阵乘 → softmax → 乘回 → IDWT），计算复杂度降到全分辨率注意力的 **1/4**；再做**特征增强**：交互结果与原特征拼接 → 通道注意力 → 空间注意力 → 拼接 → 3×3 Conv 得 $F^W$。同频分量单独交互，既省算力又避免高低频互相干扰，消融里去掉 FCE 后 MAE 上升最多（中层上下文交互贡献最大）。
