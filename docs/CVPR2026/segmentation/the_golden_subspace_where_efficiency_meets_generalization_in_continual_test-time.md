...
**本文要解决什么？**（1）证明"黄金子空间"的存在——即最小特征更新子空间与分类器权重行空间重合且低秩；（2）找到一种无需重训练分类器就能在线估计该子空间的方法；（3）设计一个实际的轻量适应框架。
...
**核心idea一句话**：特征空间中由分类器权重定义的低秩行空间就是最小有效适应子空间（黄金子空间），可用 AGOP 在线估计并高效维护。
