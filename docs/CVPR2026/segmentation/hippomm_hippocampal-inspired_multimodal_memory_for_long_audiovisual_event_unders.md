...
4. **本文要解决什么？** (a) 如何从连续音视频流中构建情景记忆？(b) 如何实现跨模态模式补全（一个模态的线索触发另一个模态的回忆）？(c) 如何在精度和效率之间取得平衡？

5. **切入角度**：生物海马体通过齿状回（DG）的模式分离、CA3 的自联想模式补全和 CA1 的记忆固化解决了上述问题。作者将这三种机制直接映射为算法实现。

6. **核心 idea 一句话**：将海马体"分割-固化-检索"的认知流程映射为"内容自适应分段 → 相似性过滤压缩 → 置信度门控层级检索"的计算架构，实现长音视频的情景记忆理解。
