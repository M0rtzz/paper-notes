...
这篇工作要解决的是：现有幻觉检测器只看全图聚合的注意力/概率/相似度，而幻觉 token 往往跟多个局部区域有「弱但分散」的相关性，聚合之后反而伪装成高相关、骗过全局检测器。它的破局思路是把视角从全图下沉到 patch 级别——逐 token 分析它与每个图像 patch 的细粒度交互，提取「注意力空间分布」和「跨模态语义对齐」两类结构性特征，再喂给一个轻量分类器判断这个 token 是不是幻觉。
