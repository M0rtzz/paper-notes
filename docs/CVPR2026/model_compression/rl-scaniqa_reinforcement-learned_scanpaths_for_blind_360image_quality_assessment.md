...
RL-ScanIQA 针对 360° 图像的一个特性：全景图在沉浸式环境里只能通过有限视口逐步看，质量感知取决于看的轨迹而非全图。以往把扫描路径生成当独立预处理、再喂给质量评估，两头不对齐，还得靠昂贵的人类眼动数据监督。本文把扫描路径生成建成序列决策过程，用 PPO 直接从质量评估反馈里学任务驱动的观看策略，让"路径生成器"和"质量评估器"两个模块端到端联合优化——生成器走一条覆盖关键区域的视口序列，评估器据此回归质量分，评估误差又反过来塑造路径。
