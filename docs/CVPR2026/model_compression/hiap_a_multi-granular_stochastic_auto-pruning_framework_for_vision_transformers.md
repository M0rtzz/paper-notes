...
HiAP 的关键不是再发明一个更聪明的重要性打分器，而是把“结构选择”本身变成可训练变量、直接嵌进 ViT 的前向传播里。作者在每个 Transformer block 内放两层门控：宏观门控（macro gate）决定一个 attention head 或一个 FFN block 要不要整体保留，微观门控（micro gate）则在保留下来的结构内部继续裁掉 value 路径中的若干维度和 FFN 中间层中的若干神经元。这样网络先学“要不要整块留着”，再学“留下来的块内部到底需要多宽”。两种粒度对应的代价类型不同——宏观剪枝省的是空结构带来的访存与整头 attention 框架成本，微观剪枝省的是活跃结构内部的精细计算量——所以作者把这两部分代价分别送进损失函数，让模型学出一种先粗后细的剪枝节奏，而不是把所有结构平均收缩一遍。

### 整体框架

整体上，HiAP 从一个标准稠密 ViT 出发，在每层 attention 和 FFN 周围挂上可学习门控参数，然后在一次训练里让这些门控自己收敛到满足算力预算的子结构。具体地，每个 attention head 配宏观门控 $g_{l,h}$、每个 FFN block 配宏观门控 $b_l$；保留下来的 head，其 value 路径各维度配微观门控 $d_{l,h,j}$，FFN 各中间神经元配微观门控 $c_{l,k}$。训练时门控不直接采样硬二值，而是用 Gumbel-Sigmoid 产生 $(0,1)$ 间的连续近似值，让梯度能穿过这些原本离散的结构决策；损失函数同时压住任务损失、宏观代价、微观代价和结构可行性约束。随着温度逐渐退火，软门控从“可探索”状态过渡到接近 0/1 的硬决策，结构也随之定型；训练结束后按 $0.5$ 阈值把门控硬化、物理裁剪对应矩阵，导出真正更小更快的子网络。值得强调的是，最终产物不是带 runtime mask 的伪稀疏模型，而是物理截断后的稠密子网络，因此不依赖专用稀疏算子，在普通硬件上也能拿到真实加速。
...
flowchart TD
    ...
    subgraph S["每个 Transformer block 内的双层门控"]
        direction TB
        B["宏观门控<br/>g 决定整头、b 决定整块是否旁路"] --> C["微观门控<br/>d 收窄 value 维度、c 收窄 FFN 神经元"]
    end
    S --> D["解析化可微代价建模<br/>保留量 → 期望 MACs（C₁/C₂/C₃）"]
    D --> E["Gumbel-Sigmoid 门控训练<br/>软门控 σ((α+ε)/τ) + STE 可微回传"]
    E --> F["结构可行性约束<br/>ReLU 保留下限，防止层坍塌"]
    F --> G["单阶段搜索与导出<br/>温度 2.0→0.5 退火 → 阈值 0.5 硬化 → 物理裁剪"]
    G --> H["稠密子网络（真实加速）"]
```
...
只剪 head 内维度或 FFN 神经元的细粒度方法，虽然能压低纸面 FLOPs，但整头 attention 和整层 FFN 的矩阵仍要逐层加载、注意力图仍要构造，真正的访存压力没被根本缓解。宏观门控直接对“整条计算路径在不在”做决策：若第 $l$ 层第 $h$ 个 head 的门控 $g_{l,h}=0$，整个头被旁路，attention 输出写为 $\text{AttnOut}_{l,h}(X)=g_{l,h}\cdot \text{Attention}(XW^Q_{l,h},XW^K_{l,h},XW^V_{l,h})$；若 $b_l=0$，整个 FFN 子层被移除，$\text{FFNOut}_{l}(X)=b_l\cdot \text{FFN}(X)$。一旦宏观结构关掉，后续整套相关矩阵的计算与数据搬运都不必发生，所以它对应的是“真实推理路径”的裁剪，而不只是账面 FLOPs 的下降。

**2. 微观门控：在保留结构内部继续瘦身**

光有宏观门控，模型只能在“整块留”和“整块删”之间二选一，表达空间太窄，要么砍得太狠掉精度、要么留得太满省不下来。微观门控让活下来的结构还能继续异构地收窄：对仍然活着的 head，不再默认保留完整 $D_h$ 维 value 通道，而是用门控向量 $d_{l,h}$ 挑出真正有用的维度，$\text{Head}'_{l,h}(X)=g_{l,h}\left[\text{softmax}\left(\frac{Q_{l,h}K_{l,h}^{\top}}{\sqrt{D_h}}\right)(V_{l,h}\odot d_{l,h})\right]$；对 FFN，用 $c_l$ 决定哪些中间神经元继续参与两层线性映射，$\text{FFN}'_l(X)=b_l\left[(\phi(XW_{1,l})\odot c_l)W_{2,l}\right]$。这样留下来的结构可以按层、按头、按神经元形成各不相同的宽度分布，给精度保留留出了更大的空间。

**3. 解析化的可微代价建模：把预算拆成能被门控直接惩罚的基础成本**

很多可微剪枝只是笼统地“正则一下 FLOPs”，优化器分不清省下来的到底是访存还是纯算力。HiAP 把可剪枝 MACs 解析地拆成三个常数项——$C_1=2ND(3D_h)+2N^2D_h$ 对应保留单个 head 的宏观开销，$C_2=2ND+2N^2$ 对应保留一个 attention value 维度的微观代价，$C_3=4ND$ 对应保留一个 FFN 神经元的代价。总期望代价则把门控的期望乘进去：

$$
\mathbb{E}[C(\mathcal{G})]=\sum_{l,h}\left(C_1\mathbb{E}[g_{l,h}]+C_2\sum_j\mathbb{E}[g_{l,h}d_{l,h,j}]\right)+\sum_{l,k}C_3\mathbb{E}[b_lc_{l,k}]。
$$

拆开以后，优化器能明确区分“空着一个 head 却不关掉”带来的浪费，和“保留 head 但少留几个维度”带来的细调收益，从而自然学出先关空结构、再压内部宽度的策略。

**4. Gumbel-Sigmoid 门控训练：把离散的“删不删”变成可微问题**

“删一个结构”本身是离散、不可导的，直接硬采样会让训练极不稳定。HiAP 给每个 gate 一个可学习 logit $\alpha$，前向时采样 Logistic 噪声 $\epsilon$，经 $\hat{z}=\sigma((\alpha+\epsilon)/\tau)$ 生成连续门值，反向则用 Straight-Through Estimator 近似传播梯度。连续松弛让这个原本只能靠启发式或搜索处理的结构决策，可以被标准的梯度优化器一并学习。

**5. 结构可行性约束：给搜索过程加结构安全护栏**

可微搜索常见的失败模式是优化早期就为了把预算项压好看而直接清空某一层，结果网络还来不及重组表征、精度就崩了。HiAP 用可行性损失限制这种行为，例如对头数写成 $\mathcal{L}_{f,\text{head}}=\sum_l \text{ReLU}(k_{\min}-\sum_h g_{l,h})^2$，当某层活跃 head 数低于下限就触发强惩罚；类似地还约束活跃 head 内至少保留一定比例维度、FFN 内至少保留一定比例神经元。它不为提精度，而是保证搜索轨迹不走向结构性坍塌。

**6. 单阶段搜索与导出：搜索、适应、导出走一条训练链路**

绝大多数可微剪枝其实把最麻烦的部分留给了后处理——先找 mask，再单独跑一轮 fine-tune，还要调阈值、回填结构。HiAP 把这些都收进同一条训练链路：初期高温度 $\tau$ 让 gate 像软 dropout 一样不断扰动结构、逼参数对不同子结构都鲁棒，后期低温度让 gate 双峰化、逐渐逼近 0/1，最后直接按门控硬化并物理裁剪权重矩阵。因为权重和结构一路 co-adapt，硬化时不会出现后剪枝那种“突然断层”，整条流程也省掉了排序、阈值搜索和二阶段微调。
