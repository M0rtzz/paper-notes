...
description: >-
  [CVPR 2026][幻觉检测][多模态幻觉] 提出一种轻量级、无需训练的插件方法，通过识别感知型和推理型注意力头并进行类别条件缩放（Class-Conditioned Rescaling），重新平衡跨层注意力分配，从而缓解多模态大推理模型（MLRM）中的幻觉问题，在5个基准上平均提升4.2%，几乎无额外推理开销。
tags:
  - "CVPR 2026"
  - "幻觉检测"
  - "多模态幻觉"
...
这篇论文把多模态幻觉归因到一个被忽视的地方：模型内部"感知"和"推理"两类功能在跨层注意力上的分配失衡，而不是单纯的视觉信息利用不足。它的做法是一个即插即用、无需训练的两步插件——先按注意力对视觉/文本 token 的分配比例和所在层深，把每个注意力头判成感知头还是推理头；再对这两类头施加大于 1 的乘法增益，放大它们的贡献来抵消浅层"感知偏差"和深层"推理漂移"。整个过程不改架构、不重训，只在每层注意力做输出投影之前轻轻缩放一下。

```mermaid
%%{init: {'flowchart': {'rankSpacing': 24, 'nodeSpacing': 28, 'padding': 6, 'wrappingWidth': 400}}}%%
flowchart TD
    A["MLRM 各层注意力头"] --> B["模态注意力比例<br/>算每个头关注视觉的占比 S_v"]
    B --> C["功能头分类规则<br/>层深 + 比例双门限"]
    C -->|"浅层且重视觉"| D["感知头"]
    C -->|"深层且轻视觉"| E["推理头"]
    C -->|"其余"| F["普通头（增益=1）"]
    D --> G["类别条件缩放<br/>感知头×g_perc、推理头×g_reas"]
    E --> G
    F --> G
    G --> H["输出投影前缩放<br/>缓解感知偏差与推理漂移"]
```

### 关键设计

**1. 模态注意力比例：量化一个头到底看不看图**

要区分感知头和推理头，先得有个能衡量"这个头有多关注视觉"的量。对每个层 $\ell$、头 $h$，定义视觉注意力分配比例 $S_v^{(\ell)}(h) = \sum_{j \in \mathcal{T}_v} a_{i^* j}^{(h,\ell)}$，即查询位置 $i^*$ 对所有视觉 token 的注意力之和；它与文本比例互补，$S_v + S_t = 1$。这个标量就是后面所有判定的基础。

**2. 功能头分类规则：层深 + 比例双门限**

感知靠浅层取证、推理靠深层符号化，这两件事既看比例也看层深。于是引入两个比例阈值 $\tau_{\text{perc}}$、$\tau_{\text{reas}}$（且 $\tau_{\text{reas}} < \tau_{\text{perc}}$）和两个层边界 $\ell_{\text{perc}}$（感知层上界）、$\ell_{\text{reas}}$（推理层下界），把头分成两类：
...
即"浅层且重视觉"的是感知头，"深层且轻视觉"的是推理头，其余的不动。

**3. 类别条件缩放：只放大、不重算注意力**

识别出功能头后，给感知头和推理头分别配一个全局增益 $g_{\text{perc}} \ge 1$、$g_{\text{reas}} \ge 1$，其余头增益为 1。关键是缩放发生在 per-head 输出算完、输出投影之前，所以并不改动底层注意力计算：

$$Y_{\text{out}}^{(\ell)} = \text{Concat}(g^{(1,\ell)} O^{(1,\ell)}, \ldots, g^{(H,\ell)} O^{(H,\ell)}) W_O^{(\ell)}$$

这样既补强了被稀释的视觉证据、又稳住了漂移的推理链，却几乎不增加推理开销。以 Ocean-R1 为例的默认配置是 $\ell_{\text{perc}}=7$、$\ell_{\text{reas}}=3$、$g_{\text{reas}}=1.30$、$\tau_{\text{reas}}=0.01$、$g_{\text{perc}}=1.16$、$\tau_{\text{perc}}=0.22$。
