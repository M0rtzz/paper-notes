...
ChartNet 的关键洞察是：图表本就是程序化生成的，所以与其在图像空间硬造数据，不如让可执行的绘图代码当结构化中间表示，把生成和增强都搬到代码空间里做。整条数据生产线绕着这个思路转：先让 VLM 把种子图表"逆向"成可执行绘图代码，再用 LLM 反复重写代码生成多样变体，执行渲染出图像，过一道视觉质量过滤把废品筛掉，最后结合图像和代码两路上下文抽出数据表、文本描述和带推理的 QA，凑成五元组对齐样本。
