...
4. **本文要解决什么？** 设计一种**不依赖特定触发器类型**的通用后门清洗方法，且仅通过微调（而非从头训练）实现，使计算成本可控。

5. **切入角度**：作者的关键洞察是——与其试图猜测和覆盖所有可能的触发器模式，不如直接把模型"推离"中毒状态。只要模型的 embedding 空间与中毒模型足够不同，后门带来的虚假关联自然就被破坏了。

6. **核心 idea 一句话**：用 $\ell_2$ 距离损失主动推离中毒 embedding（Perturb），同时用 CLIP 对比损失保持干净性能（Recover），两者竞争实现"忘掉后门、记住知识"。
