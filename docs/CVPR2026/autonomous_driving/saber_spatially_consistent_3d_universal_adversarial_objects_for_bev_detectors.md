...
SABER 要回答的问题是：纯视觉 BEV 检测器能不能被一个放在车旁、不接触目标的 3D 物体骗倒。它的 pipeline 分三步：先在 3D 场景里自动给对抗 mesh 选个合适位置，再用可微渲染把它渲成多视角一致的对抗图像、并用遮挡感知模块保证物理合理，最后通过 BEV 空间特征引导的优化让攻击跨视角、跨帧都有效。
