...
description: >-
  [CVPR 2026][自动驾驶][视觉几何] DVGT 是一个面向自动驾驶的视觉几何 Transformer，输入一段无位姿的多帧多视角图像，端到端直接预测以首帧自车坐标系为基准的**度量尺度**全局稠密 3D 点云图与每帧自车位姿，无需相机内外参、无需事后用 LiDAR 对齐尺度，在五个驾驶数据集上同时超越通用几何模型（VGGT、CUT3R、MapAnything）和驾驶专用模型（Driv3R）。
tags:
  - "CVPR 2026"
  - "自动驾驶"
  - "视觉几何"
