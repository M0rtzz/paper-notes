...
简单的 copy-paste 会留下边界伪影，被分割模型当成捷径学走；这个模块要解决的就是「怎么把异常物体放得物理合理」。它走五步：① 在语义图的可驾驶区域（Drivable Region）均匀采样 64 个候选位置，生成 pseudo bounding boxes；② 按透视先验调整 bbox 大小——根据物体在图像中的垂直位置 $y_i$ 缩放高度 $h_i = \frac{H}{y_i}$，越靠图像上方（越远）物体越小，避免远处放巨物、近处放微物的违和感；③ 用检测 backbone 对候选 bbox 预测置信度，再通过匈牙利匹配（Hungarian Matching）挑出互不重叠、布局自然的最优位置；④ 在选定位置用扩散模型做 inpainting（以异常类别为 prompt），让物体的光照、风格与场景一致；⑤ 用 Grounding-SAM 对 inpaint 区域生成精确分割 mask，直接作为训练 GT。整条链路把「采样—透视—匹配—生成—打标」自动化，既保证物理真实性，又省掉了人工标注。
