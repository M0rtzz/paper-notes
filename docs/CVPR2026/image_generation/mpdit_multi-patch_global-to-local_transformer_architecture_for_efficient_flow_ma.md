...
4. **本文要解决什么？** 如何在保持生成质量的前提下显著降低扩散 Transformer 的计算量和训练成本？

5. **切入角度**：受全局-局部注意力的启发，但不是在注意力层级实现（效果不好且增益可忽略），而是在整个架构层级实现——前面的层看"粗粒度"全局，后面的层看"细粒度"局部。

6. **核心idea一句话**：将等距 DiT 改为"先粗后细"的层次化结构——多数 Transformer 块在大 patch（64 token）上高效获取全局语义，少量尾部块在小 patch（256 token）上精修局部细节。
