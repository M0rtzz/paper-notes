...
**1. 簇级源分布识别：用模型自身的可逆性 bootstrap 出源分布**

全局 OT 复杂度是样本数的立方、根本算不动，得先把数据分簇。作者利用预训练 FM 模型的可逆性，对簇 $\mathcal{C}_k$ 里每个数据样本 $x_1$ 反转 ODE 追回源样本 $\hat{x}_0 := x_1 - \int_0^1 v_\theta(\hat{x}_t, t) \, dt$，再把这批回溯样本 $\hat{X}_{0,k}$ 近似成高斯 $p_{0,k}(x) = \mathcal{N}(x; \boldsymbol{\mu}_{0,k}, \boldsymbol{\Sigma}_{0,k})$。妙处在于预训练模型的路径天然不交叉，反转得到的源分布自然保持簇间分离，不需要任何额外标注。

**2. 簇内最优传输近似：把不可解的全局 OT 拆成 K 个小 OT**
...
分簇方式按场景灵活选择——条件生成直接用类别标签；无条件（CIFAR-10）用 DINO 特征 + K-Means（K=100）；机器人策略这种非固定聚类则学一个模块来预测新观测的源分布。
