...
description: >-
  [CVPR2026][图像生成][图像复原] 提出 Face2Scene 两阶段框架：先用参考人脸复原模型(Ref-FR)获得 HQ-LQ 人脸对，从中提取退化编码作为"oracle"，再以此条件化单步扩散模型完成包含身体与背景的全场景图像复原。 参考人脸复原局限于面部区域：现有 Ref-FR 方法仅复原人脸裁剪区域…
tags:
  - "CVPR2026"
  - "图像生成"
  - "图像复原"
...
flowchart TD
    A["LQ 全场景图 + 同身份参考图"] --> B["阶段一：参考人脸复原 Ref-FR<br/>借参考图复原检测到的人脸"]
    B --> C["空间对齐的人脸对 (HQ, LQ)"]
    C --> D["FaDeX<br/>编码器吃 6 通道 → 退化编码 q (oracle)"]
    D --> E["MapNet<br/>映射成 21 个多尺度退化 token"]
    E --> F["注入单步扩散 SD-Turbo cross-attention"]
    A --> F
    F --> G["HQ 全场景复原图"]
```
