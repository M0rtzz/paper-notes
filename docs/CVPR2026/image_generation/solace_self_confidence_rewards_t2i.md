...
给定文本 prompt $c$，一轮的流程是：先采样 $G=16$ 组独立反向轨迹得到一批终端潜表示 $\{z_0^{(i)}\}_{i=1}^G$；抽 $K=8$ 个共享噪声探针 $\epsilon^{(m)} \sim \mathcal{N}(0,I)$（反义配对保证均值为零）；对每个 $z_0^{(i)}$ 在若干时间步 $t \in \mathcal{T}$ 按 $z_t^{(i,m)} = (1-t)z_0^{(i)} + t\epsilon^{(m)}$ 重新加噪；让模型预测速度场 $v_\theta(z_t^{(i,m)}, t, c)$ 并还原噪声估计 $\hat{\epsilon}_\theta = v_\theta + z_0^{(i)}$；最后用恢复误差算出自信心奖励送进 GRPO。
