...
description: >-
  [ACL 2026 Findings][信息检索/RAG][检索增强生成] 本文从隐藏表示的角度系统分析 RAG 中检索文档如何影响 LLM 内部状态，发现了五个关键模式：随机文档引发大表示漂移并触发拒绝行为、相关文档主要确认而非改变参数化知识、单个相关文档能锚定多文档场景中的表示、后层逐步强调参数化知识从而限制检索证据的影响、以及 LLM 在早期层就能区分随机文档但到最后层仍无法可靠区分干扰文档和相关文档。
tags:
  - "ACL 2026 Findings"
  - "信息检索/RAG"
  - "检索增强生成"
...
本文是一项机制剖析工作，不训练模型，而是用受控实验把 RAG 的黑箱打开到隐藏表示层面。固定 RAG 管道后，对每个查询系统变化文档的相关性类型（相关 / 干扰 / 随机）与数量组合，再把“最后一个 prompt token”在各层的隐藏状态 $h \in \mathbb{R}^{L \times D}$ 抽出来，对比有 / 无上下文时的表示偏移。整个流程从查询 + 检索文档进去，经过检索层（MassiveDS 库 + Contriever 取 top-20 并由 GPT-5 分类），最后用 PCA 可视化、余弦相似度与表示可分性输出“哪类文档在哪些层改变了内部状态”。实验跨 4 个 QA 数据集（TriviaQA、NQ、PopQA、StrategyQA）× 3 个 LLM（Gemma3-27B、Llama4-17B、Qwen3-Next-80B）。

### 关键设计

**1. 受控文档分类体系：把“检索混合体”拆成三种可隔离的自变量**

真实检索场景下取回的文档是相关、干扰、随机三类混在一起的，不拆开就无法归因到具体是哪类文档驱动了表示变化。本文把文档严格定义为三类：相关文档含真实答案或直接支撑答案的信息；干扰文档与查询语义相似但不含支撑信息、可能误导模型；随机文档与查询语义相似度低、完全无用。分类由 GPT-5 完成并经人工验证，这是后续所有机制结论能成立的前提。

**2. 查询难度分层：区开“本来就会”和“靠检索才会”两种场景**

模型已有参数化知识时和缺乏知识时，对检索文档的处理机制可能完全不同，混在一起统计会抹掉这种差异。本文对每个查询先用纯查询（无检索）测一遍：能答对的标为 easy，否则为 hard。这样就能分开考察检索文档对“已知知识”和“未知知识”的不同影响——后面会发现随机文档竟然能把本来答得对的 easy 查询推向拒绝回答，这个区分是看出现象的关键。

**3. 层级表示分析：顺着 Transformer 的层看检索信息在哪里被整合或覆盖**

Transformer 不同层职能不同，浅层做词法处理、中层做语义整合、深层做决策，只看最终输出看不出检索证据究竟在哪一段起作用。本文在每一层抽出最后一个 prompt token 的隐藏状态，用 PCA 可视化不同上下文类型的表示分布并追踪从浅到深的演化轨迹。正是这一分析揭示了“早期层（约 L23）就能分开随机文档、但相关与干扰文档直到最后层仍混在一起”，以及“后层逐步把相关文档表示拉回无文档表示、说明深层更强调参数化知识”这些关键观察。

### 损失函数 / 训练策略

本文为分析性工作，不涉及模型训练，使用预训练的 LLM（指令微调与基础版本）进行推理和表示提取。
