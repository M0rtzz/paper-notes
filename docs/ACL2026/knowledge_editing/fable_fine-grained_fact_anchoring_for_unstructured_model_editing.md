...
description: >-
  [ACL 2026 Findings][知识编辑][模型编辑] 本文发现现有非结构化模型编辑方法虽能整体性回忆编辑文本但无法进行细粒度事实访问，提出FABLE框架通过两阶段层次化策略将细粒度事实锚定到浅层、整体性叙事整合到深层，并构建UnFine诊断基准进行系统评估。 领域现状：模型编辑旨在通过修改少量参数来更新LLM的特定…
tags:
  - "ACL 2026 Findings"
  - "知识编辑"
  - "模型编辑"
