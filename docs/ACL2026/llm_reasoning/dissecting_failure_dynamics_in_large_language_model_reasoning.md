...
flowchart TD
    A["推理主轨迹<br/>逐 token 生成"] --> B["实时监控 token 级熵"]
    B --> C{"基于实例自适应阈值的失败检测<br/>步骤边界熵 > 历史 q 分位数?"}
    C -->|"未超阈值"| F
    C -->|"检测到转折点"| D["短距语义分支<br/>3 条续写：动量 / 抑制 / 反事实"]
    D --> E["选平均熵最低的续写<br/>接回单条主轨迹"]
    E --> F{"后期截断<br/>剩余容量 ρ ≤ ρ_min 且遇犹豫标记?"}
    F -->|"否·继续生成"| B
    F -->|"是"| G["替换为终止信号<br/>掐断认知螺旋"]
    G --> H["输出最终答案"]
```

### 关键设计

**1. 推理失败动态的四个发现：先把「错误长什么样」摸清楚，再决定在哪里下刀**

设计干预之前，作者对错误轨迹做了逐段解剖，得到四条彼此咬合的规律。其一是**失败早发性**：85% 以上的失败起点落在轨迹前 30%，而且 43.5% 的错误轨迹只含单个错误段——错误不是均匀散布的，而是早早地在少数关键转折点爆发。其二是**认知螺旋**：错误发生后轨迹反而显著变长，并持续保持局部连贯，形成一种「看似合理、实则全局错误」的越陷越深。其三是**熵信号**：失败起点处的 token 级熵会出现局部尖峰，错误段整体熵显著高于正确段（$p<0.001$），这给了检测一个现成的抓手。其四是**局部可恢复性**：20% 以上的失败轨迹，从同一前缀换一条续写就能走到正确答案。这四条合起来说明一件事——错误是局部的、可检测的、且有相当一部分可修复，因此「只在关键位置动手」比「全局加计算」更划算，整套 GUARD 都建立在这个结论上。

**2. 基于实例自适应阈值的失败检测：用历史熵的分位数判定转折点，避开绝对阈值的尺度陷阱**

知道熵尖峰是失败信号后，关键是怎么判定「这个尖峰算不算高」。作者只在推理步骤边界（分隔符）处检查，判据是当前 token 熵是否超过此前历史熵的 $q$ 分位数：

$$\mathbb{I}_{drift}(x_t) = \mathbb{I}[x_{t-1} \in \mathcal{T}_{delim} \land \mathcal{H}(x_t) > \text{Quantile}_q(\mathbf{H}_{<t})]$$

之所以用分位数而非一个固定的绝对阈值，是因为简单题的「高熵」可能还不及难题的「正常熵」——绝对阈值在不同难度的问题上根本不通用。改用相对于本题历史熵分布的分位数，检测就自动适配了每道题各自的熵尺度，简单题难题都能用同一套判据稳定触发。

**3. 短距语义分支与后期截断：在偏离点做小手术，而不是维护多条完整并行路径**

检测到风险点后，GUARD 不去铺开多条完整轨迹（那是 Best-of-N 的高成本做法），而是只生成 3 条**短距**续写：动量分支（标准贪心续写）、抑制分支（前置「Wait,」打断当前模式）、反事实分支（前置「Let me reconsider:」促使重新考虑），然后选平均熵最低的那条接回单条主轨迹继续走。这正是顺着「局部可恢复性」那条发现来的——既然换条短续写就有两成机会救回来，那就不必探索完整替代路径，只要在偏离点给几个局部选项、挑最确定的一个。到推理后期，当剩余容量 $\rho_t \leq \rho_{min}$ 时，再遇到犹豫标记就直接替换成终止信号，掐掉那些只会让认知螺旋继续延伸的无效 token。三条分支共享预计算的 KV 缓存，额外延迟被压到最低。
