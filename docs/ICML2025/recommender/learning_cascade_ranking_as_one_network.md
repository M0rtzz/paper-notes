...
**本文要解决什么？**: 设计能同时解决目标不对齐和缺乏协同学习的端到端训练范式，使所有阶段作为一个统一网络联合优化。

**切入角度**: 将级联排序的目标形式化为ground truth items通过所有阶段的存活概率最大化问题，利用可微排序技术将离散的top-k选择松弛为连续概率，推导下界后直接优化。

**核心idea一句话**: 用可微排序的软置换矩阵构建各阶段的top-k选择概率，将级联的联合存活概率分解为各阶段概率之积的下界$\hat{P}_{CS}^{q_2} = \prod_i P_{\mathcal{M}_i}^{q_i}$，直接优化该下界实现端到端对齐，并从下界紧致度推导出辅助损失促进阶段间一致性。
