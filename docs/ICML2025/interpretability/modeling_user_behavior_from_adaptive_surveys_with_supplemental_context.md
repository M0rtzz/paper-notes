...
**本文要解决什么？**: 设计一个架构使调查数据保持中心地位，仅在外部信号有助于行为预测时选择性地融入，且模块化支持新数据源的灵活集成。

**切入角度**: 采用后期融合（late fusion）策略，用调查嵌入作为query查询外部嵌入——这意味着外部信息只在调查问"需要什么"时才被检索，而非不加区分地混合。

**核心idea一句话**: 用调查嵌入作为交叉注意力的query、外部数据嵌入作为key/value实现后期融合，加上选择性门控$g$和残差连接$h_{fused} = h_s + g \odot (h_t - h_s)$确保调查信号主导，构建"调查主导、补充辅助"的用户行为建模范式。
