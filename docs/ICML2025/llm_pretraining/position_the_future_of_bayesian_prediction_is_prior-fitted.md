...
**本文要解决什么**：论证一种新的贝叶斯预测范式——PFN——可以 (a) 充分利用大规模预训练算力，(b) 支持声明式先验定义（只需能采样），(c) 推理仅需一次前向传播，(d) 覆盖传统方法无法触及的先验类别。

**切入角度**：从 TabPFN 在表格数据上超越 XGBoost 的成功出发，作者观察到这种"在合成数据上预训练 + 在真实数据上上下文学习"的范式具有通用的方法论价值，适合推广到更多领域。

**核心 idea 一句话**：PFN 通过将贝叶斯推断转化为"在先验采样的合成数据上训练神经网络"的监督学习问题，实现了贝叶斯预测的摊还化（amortization），是算力充沛、数据稀缺时代最合适的贝叶斯方法。
