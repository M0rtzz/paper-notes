...
**本文要解决什么？** 无需训练数据，将任意预训练 LoRA 从源模型迁移到目标模型。

**切入角度**：SVD 分解源/目标模型权重，将 LoRA 的子空间和零空间分量分别投影到目标模型的对应空间。

**核心idea一句话**：$\Delta W_{t\leftarrow s} = U_{t,\parallel}U_{t,\parallel}^\top \Delta W_{s,\parallel} V_{t,\parallel}^\top V_{t,\parallel} + U_{t,\perp}U_{t,\perp}^\top \Delta W_{s,\perp} V_{t,\perp}^\top V_{t,\perp}$
