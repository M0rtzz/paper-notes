...
**本文要解决什么？** 在攻击者「不知道类名」且「没有 prompt 设计能力」的更现实威胁模型下，如何自动化地进行高效模型窃取？具体子问题：
...
**核心 idea 一句话**：用遗传算法进化 prompt 种群，以受害模型对合成图像的分类一致性（Prompt Consistency）作为适应度函数，迭代优化 prompt 的精准度和多样性。
