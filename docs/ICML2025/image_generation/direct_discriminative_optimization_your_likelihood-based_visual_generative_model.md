...
**本文要解决什么**：在不改变网络结构、不增加推理成本的前提下，利用 GAN 目标对预训练的似然生成模型进行微调，突破 MLE 的质量上限。

**切入角度**：关键洞察——似然模型本身就能充当 GAN 判别器！通过两个似然模型的似然比来隐式参数化判别器，类似 DPO 中用策略对数比来参数化奖励模型。

**核心 idea 一句话**：用 $d_\theta(\mathbf{x}) = \sigma\left(\log \frac{p_\theta(\mathbf{x})}{p_{\theta_{\text{ref}}}(\mathbf{x})}\right)$ 隐式参数化判别器，将 GAN 训练简化为直接微调生成模型本身。
