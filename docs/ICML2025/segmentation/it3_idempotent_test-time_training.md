...
description: >-
  [ICML 2025][语义分割][测试时训练] 提出 IT³，一种基于幂等性（idempotence）的通用测试时训练方法，通过最小化网络递归调用间的偏差来适应分布外样本，无需领域特定的辅助任务，适用于任意任务和架构。 领域现状：测试时训练（TTT）在推理阶段对每个测试样本进行短暂的模型微调以提升分布外（OOD）性能…
tags:
  - "ICML 2025"
  - "语义分割"
  - "测试时训练"
