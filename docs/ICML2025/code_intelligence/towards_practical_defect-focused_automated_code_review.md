...
description: >-
  [ICML 2025][代码智能][自动代码审查] 提出面向缺陷检测的端到端自动代码审查框架，通过代码切片、多角色LLM系统、冗余过滤和行号定位四大模块，在真实工业级C++代码库上实现2倍于标准LLM、10倍于先前基线的缺陷检测性能提升。
tags:
  - ICML 2025
  - 代码智能
  - 自动代码审查
...
**做什么**：从diff hunk出发，利用AST静态分析，按不同粒度提取相关代码上下文。
...
**做什么**：将代码格式化为 `+行号 {新增代码}` / `-行号 {删除代码}` / `... ...`（省略非关键行），嵌入prompt中，让LLM输出评论时直接引用行号。
